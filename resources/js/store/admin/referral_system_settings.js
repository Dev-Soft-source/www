...
const settings = {
    ...
    state: {
        ...
        form: {
            id: null,
            p_2_p_booking_credit: null,
            p_2_s_booking_credit: null,
            p_2_d_booking_credit: null,
            d_2_p_reward_point: null,
            d_2_s_reward_point: null,
            d_2_d_rewad_point: null,
            s_2_p_reward_point: null,
            s_2_s_reward_point: null,
            s_2_d_reward_point: null,
        },
        settings: null,
        loading: false,
        sortBy: "id",
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            let method = "put";
            let url = `${process.env.MIX_ADMIN_API_URL}referral-system-settings/${state.form.id}`;
            commit("setLoading");
        ...
        fetchSetting({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}referral-system-settings/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
