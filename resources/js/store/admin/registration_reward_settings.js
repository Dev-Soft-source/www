...
const settings = {
    ...
    state: {
        ...
        form: {
            id: null,
            passenger_credit_booking: null,
            driver_reward_point: null,
            student_reward_point: null,
        },
        settings: null,
        loading: false,
        sortBy: "id",
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            let method = "put";
            let url = `${process.env.MIX_ADMIN_API_URL}registration-reward-settings/${state.form.id}`;
            commit("setLoading");
        ...
        fetchSetting({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}registration-reward-settings/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
