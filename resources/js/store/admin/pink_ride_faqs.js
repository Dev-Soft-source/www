...
const pinkRideFaqs = {
    ...
    mutations: {
        ...
        setpinkRideFaqs(state, payload) {
            state.pinkRideFaqs = payload;
        },
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}pink-ride-faqs/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}pink-ride-faqs`;
            commit("setLoading");
        ...
        fetchPinkRideFaqs({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}pink-ride-faqs?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setpinkRideFaqs", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchPinkRideFaq({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}pink-ride-faqs/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deletePinkRideFaq({ commit }, payload) {
            commit("setLoading");
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}pink-ride-faqs/${payload.id}`;
            return new Promise(function (resolve, reject) {
                axios.delete(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
