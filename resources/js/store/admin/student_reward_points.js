    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}student-reward-points/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}student-reward-points`;
            commit("setLoading");
        ...
        fetchStudentRewardPoints({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}student-reward-points?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        fetchStudentRewardPoint({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}student-reward-points/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteStudentRewardPoint({ commit }, payload) {
            commit("setLoading");
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}student-reward-points/${payload.id}`;
            return new Promise(function (resolve, reject) {
                axios.delete(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
