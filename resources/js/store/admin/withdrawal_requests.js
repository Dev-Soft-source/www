...
const withdrawals = {
    ...
    mutations: {
        ...
        updateWithdrawalById(state, { id, status }) {
            const withdrawalToUpdate = state.withdrawals.find(withdrawal => withdrawal.id === id);
    ...
    actions: {
        fetchWithdrawals({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}withdrawal-requests?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
                ...
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setWithdrawals", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        async rejectWithdrawal({ commit }, payload) {
            ...
            try {
              const url = payload && payload.url
                ? payload.url
                : `${process.env.MIX_ADMIN_API_URL}reject-withdrawal-request/${payload.id}`;
              const res = await axios.put(url);

              if (res && res.data && res.data.status === "Success") {
                // Update the user from the state
                commit("updateWithdrawalById", { id: payload.id, status: 2 });
