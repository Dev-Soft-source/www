...
const verify_phones = {
    ...
    mutations: {
        setVerifyPhones(state, payload) {
            state.verify_phones = payload;
        },
        ...
        updateWithdrawalById(state, { id, status }) {
            const withdrawalToUpdate = state.verify_phones.find(withdrawal => withdrawal.id === id);
    ...
    actions: {
        fetchVerifyPhones({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}verify-phones?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
                ...
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setVerifyPhones", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        async rejectWithdrawal({ commit }, payload) {
            ...
            try {
              const url = payload && payload.url
                ? payload.url
                : `${process.env.MIX_ADMIN_API_URL}verify-phone-request/${payload.id}`;
              const res = await axios.put(url);
