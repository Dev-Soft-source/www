...
const rewardPointSettings = {
    ...
    mutations: {
        ...
        setRewardPointSettings(state, payload) {
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}passenger-reward-points/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}passenger-reward-points`;
            commit("setLoading");
        ...
        fetchPassengerRewardPoints({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}passenger-reward-points?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setRewardPointSettings", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchPassengerRewardPoint({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}passenger-reward-points/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deletePassengerRewardPoint({ commit }, payload) {
            commit("setLoading");
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}passenger-reward-points/${payload.id}`;
            return new Promise(function (resolve, reject) {
                axios.delete(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
