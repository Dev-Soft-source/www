...
const settings = {
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            let method = "put";
            let url = `${process.env.MIX_ADMIN_API_URL}folk-ride-settings/${state.form.id}`;
            commit("setLoading");
        ...
        fetchSetting({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}folk-ride-settings/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
