const students = {
    ...
    mutations: {
        ...
        removeStudentById(state, studentId) {
            state.students = state.students.filter(student => student.id !== studentId);
        },
        updateUserStatus(state, { id, suspand }) {
            const userToUpdate = state.students.find(student => student.id === id);
            if (userToUpdate) {
    ...
    actions: {
        fetchStudents({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}student-verification?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
        ...
        async deleteStudent({ commit }, payload) {
            ...
            try {
              ...
              if (res && res.data && res.data.status === "Success") {
                ...
                commit("removeStudentById", payload.id);
