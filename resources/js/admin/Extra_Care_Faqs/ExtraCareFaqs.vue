            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Extra-Care FAQs</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.extra-care-faqs.create' }"
                ...
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-banks"
                            class="block  pl-10  w-full md:w-80 bg-white can-exp-input" placeholder="Search for faq"
                            v-model="quickSearch" />
                    </div>
                </div>
                        ...
                        <div class="bg-white shadow-lg hover:shadow-xl rounded-md overflow-x-auto">
                            <table
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="extraCareFaq in extraCareFaqs" :key="extraCareFaq.id"
                                        ...
                                        <td class="py-2 md:p-3 md:border-none relative">
                                            <div class="font-medium text-gray-900 flex mt-1 items-center gap-1">
                                                <span>{{ extraCareFaq.extra_care_faq_detail[0]?.question }}</span>
                                            </div>
                                        </td>
                                        <td
                                            class="p-2 md:p-3 gap-1 md:gap-2 justify-center items-center absolute top-4 md:top-0 right-0 md:relative flex">
                                            <!-- <router-link
                                                :to="{ name: 'admin.extra-care-faqs.edit', params: { id: extraCareFaq.id } }"
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.extra-care-faqs.edit', params: { id: extraCareFaq.id } }).href"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="deleteExtraCareFaq(extraCareFaq)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click="fetchExtraCareFaqs(pagination.prev_page_url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="fetchExtraCareFaqs(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchExtraCareFaqs(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchExtraCareFaqs(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchExtraCareFaqs(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            extraCareFaqs: (state) => state.extraCareFaqs.extraCareFaqs,
            pagination: (state) => state.extraCareFaqs.pagination,
            validationErros: (state) => state.extraCareFaqs.validationErros,
            searchParam: (state) => state.extraCareFaqs.searchParam,
            loading: (state) => state.extraCareFaqs.loading,
        }),
        limit: {
            get() {
                return this.$store.state.extraCareFaqs.limit;
            },
            set(value) {
                this.$store.commit('extraCareFaqs/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchExtraCareFaqs(page_url) {
            this.$store.dispatch("extraCareFaqs/fetchExtraCareFaqs", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("extraCareFaqs/setLimit", value);
            this.$store.dispatch("extraCareFaqs/fetchExtraCareFaqs");
        },
        deleteExtraCareFaq(extraCareFaq) {
            this.$swal
                .fire({
                    title: "Are you sure?",
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("extraCareFaqs/deleteExtraCareFaq", {
                            id: extraCareFaq.id,
                        })
                            .then((res) => {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("extraCareFaqs/setSearchParam", this.quickSearch);
            this.$store.dispatch("extraCareFaqs/fetchExtraCareFaqs");
        }, 500),
    },
    created() {
        this.$store.commit("extraCareFaqs/setLimit", 100);
        this.$store.commit("extraCareFaqs/setSortBy", "id");
        this.$store.commit("extraCareFaqs/setSortType", "desc");
        this.$store.commit("extraCareFaqs/setSearchParam", '');
        this.$store.dispatch("extraCareFaqs/fetchExtraCareFaqs");
    },
    watch: {
