                ...
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h3 class="can-exp-h2 text-primary">
                            {{ isFormEdit ? "Edit" : "Create" }} driver reward point
                        </h3>
                        <router-link
                            :to="{ name: 'admin.driver-reward-points.index' }"
...
export default {
    computed: {
        ...mapState({
            ...
            form: (state) => state.rewardPointSettings.form,
            isFormEdit: (state) => state.rewardPointSettings.isFormEdit,
        ...
        is_default: {
            ...
            set: function (val) {
                this.$store.commit("rewardPointSettings/setForm", {
                    is_default: val,
                });
            },
        },
    ...
    methods: {
        handleInput(value, key) {
            this.$store.commit("rewardPointSettings/setState", { key, value });
        },
        handleMultipleInput(key, value, language) {
            this.$store.commit("rewardPointSettings/updateState", {
                value: value,
                id: language.id,
                key,
            });
        ...
        addUpdateForm() {
            this.$store
                .dispatch("rewardPointSettings/addUpdateForm")
                .then(() =>
                    this.$router.push({ name: "admin.driver-reward-points.index" })
                );
        },
        changeLanguageTab(language) {
            this.activeTab = language.id;
        },
        fetchDriverRewardPoint() {
            if (this.$route.params.id) {
                let id = this.$route.params.id;

                this.$store.commit("rewardPointSettings/setIsFormEdit", 1);
                this.$store
                    .dispatch("rewardPointSettings/fetchDriverRewardPoint", {
                        id: id,
                        url: `${process.env.MIX_ADMIN_API_URL}driver-reward-points/${id}?withRewardPointSettingDetail=1`,
                    })
                    .then((res) => {
                        let keys = [
                            "page",
                        ];
                        this.$store.commit("rewardPointSettings/setState", {
                            key: "id",
                            value: id,
                        });
    ...
    created() {
        this.$store.commit("rewardPointSettings/resetForm");
        this.$store
