            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Driver Reward Points</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.driver-reward-points.create' }"
                            class="block button-exp-fill w-full">
                            Add new driver reward point
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-banks"
                            ...
                            placeholder="Search for driver reward point" v-model="quickSearch" />
                    </div>
                </div>
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="rewardPointSetting in rewardPointSettings" :key="rewardPointSetting.id"
                                        ...
                                        <td
                                            ...
                                            <!-- <router-link
                                                :to="{ name: 'admin.driver-reward-points.edit', params: { id: rewardPointSetting.id } }"
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.driver-reward-points.edit', params: { id: rewardPointSetting.id } }).href"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="deleteDriverRewardPoint(rewardPointSetting)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click="fetchDriverRewardPoints(pagination.prev_page_url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="fetchDriverRewardPoints(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchDriverRewardPoints(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchDriverRewardPoints(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchDriverRewardPoints(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            rewardPointSettings: (state) => state.rewardPointSettings.rewardPointSettings,
            pagination: (state) => state.rewardPointSettings.pagination,
            validationErros: (state) => state.rewardPointSettings.validationErros,
            searchParam: (state) => state.rewardPointSettings.searchParam,
            loading: (state) => state.rewardPointSettings.loading,
        }),
        limit: {
            get() {
                return this.$store.state.rewardPointSettings.limit;
            },
            set(value) {
                this.$store.commit('rewardPointSettings/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchDriverRewardPoints(page_url) {
            this.$store.dispatch("rewardPointSettings/fetchDriverRewardPoints", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("rewardPointSettings/setLimit", value);
            this.$store.dispatch("rewardPointSettings/fetchDriverRewardPoints");
        },
        deleteDriverRewardPoint(rewardPointSetting) {
            this.$swal
                .fire({
                    title: "Are you sure?",
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("rewardPointSettings/deleteDriverRewardPoint", {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("rewardPointSettings/setSearchParam", this.quickSearch);
            this.$store.dispatch("rewardPointSettings/fetchDriverRewardPoints");
        }, 500),
    },
    created() {
        this.$store.commit("rewardPointSettings/setLimit", 100);
        this.$store.commit("rewardPointSettings/setSortBy", "id");
        this.$store.commit("rewardPointSettings/setSortType", "desc");
        this.$store.commit("rewardPointSettings/setSearchParam", '');
        this.$store.dispatch("rewardPointSettings/fetchDriverRewardPoints");
    },
    watch: {
