            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Pink-Ride FAQs</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.pink-ride-faqs.create' }" class="block button-exp-fill w-full">
                            Add new FAQ
                        </router-link>
                    </div>
                </div>
                        ...
                        <div class="bg-white shadow-lg hover:shadow-xl rounded-md overflow-x-auto">
                            <table
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="pinkRideFaq in pinkRideFaqs" :key="pinkRideFaq.id"
                                        ...
                                        <td class="py-2 md:p-3 md:border-none relative">
                                            <div class="font-medium text-gray-900 flex mt-1 items-center gap-1">
                                                <span>{{ pinkRideFaq.pink_ride_faq_detail[0]?.question }}</span>
                                            </div>
                                        </td>
                                        <td
                                            class="p-2 md:p-3 gap-1 md:gap-2 justify-center items-center absolute top-4 md:top-0 right-0 md:relative flex">
                                            <!-- <router-link
                                                :to="{ name: 'admin.pink-ride-faqs.edit', params: { id: pinkRideFaq.id } }"
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.pink-ride-faqs.edit', params: { id: pinkRideFaq.id } }).href"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="deletePinkRideFaq(pinkRideFaq)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchPinkRideFaqs(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchPinkRideFaqs(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchPinkRideFaqs(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            pinkRideFaqs: (state) => state.pinkRideFaqs.pinkRideFaqs,
            pagination: (state) => state.pinkRideFaqs.pagination,
            validationErros: (state) => state.pinkRideFaqs.validationErros,
            searchParam: (state) => state.pinkRideFaqs.searchParam,
            loading: (state) => state.pinkRideFaqs.loading,
        }),
        limit: {
            get() {
                return this.$store.state.pinkRideFaqs.limit;
            },
            set(value) {
                this.$store.commit('pinkRideFaqs/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchPinkRideFaqs(page_url) {
            this.$store.dispatch("pinkRideFaqs/fetchPinkRideFaqs", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("pinkRideFaqs/setLimit", value);
            this.$store.dispatch("pinkRideFaqs/fetchPinkRideFaqs");
        },
        deletePinkRideFaq(pinkRideFaq) {
            this.$swal
                .fire({
                    title: "Are you sure?",
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("pinkRideFaqs/deletePinkRideFaq", {
                            id: pinkRideFaq.id,
                        })
                            .then((res) => {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("pinkRideFaqs/setSearchParam", this.quickSearch);
            this.$store.dispatch("pinkRideFaqs/fetchPinkRideFaqs");
        }, 500),
    },
    created() {
        this.$store.commit("pinkRideFaqs/setLimit", 100);
        this.$store.commit("pinkRideFaqs/setSortBy", "id");
        this.$store.commit("pinkRideFaqs/setSortType", "desc");
        this.$store.commit("pinkRideFaqs/setSearchParam", '');
        this.$store.dispatch("pinkRideFaqs/fetchPinkRideFaqs");
    },
    watch: {
