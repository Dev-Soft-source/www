            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Videos</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.videos.create' }" class="block button-exp-fill w-full">
                            Add video
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-videos"
                            class="block  pl-10  w-full md:w-80 bg-white can-exp-input" placeholder="Search for videos"
                            v-model="quickSearch" />
                    </div>
                </div>
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr class="border-t first:border-t-0 flex p-3 md:p-3  md:table-row flex-col w-full flex-wrap even:bg-gray-50 odd:bg-white"
                                        ...
                                        <td class="p-2 md:p-3 border-b md:border-none">
                                            <div class="">
                                                {{ video.video_detail && video.video_detail[0] ? video.page : '' }}
                                            </div>
                                        </td>
                                        <td class="p-2 md:p-3 gap-2 justify-center items-center hidden md:flex">
                                            <!-- <router-link :to="{ name: 'admin.videos.edit', params: {id: video?.id} }" type="button" class="inline-flex items-center button-exp-fill">
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.videos.edit', params: { id: video?.id } }).href"
                            ...
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                              ...
                              <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination" v-if="pagination.next_page_url || pagination.prev_page_url">
                                    <a href="#" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50" v-bind:class="[{disabled: !pagination.prev_page_url}]" @click="fetchVideos(pagination.prev_page_url)">
                                    ...
                                    <a href="#" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50" v-bind:class="[{disabled: !pagination.next_page_url}]" @click.prevent="fetchVideos(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchVideos(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchVideos(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchVideos(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    methods: {
        ...
        updateLimit(value) {
            this.$store.commit("videos/setLimit", value);
            ...
            this.$swal
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("videos/deleteVideo", {
                            id: video.id,
                        })
                            .then((res) => {
    ...
    created() {
        ...
        this.$store.commit("videos/setSortBy", "id");
        this.$store.commit("videos/setSortType", "desc");
        this.$store.dispatch("videos/fetchVideos");
    },
    watch: {
