<template>
    <AppLayout>
        <section class="forgot-password-section relative md:top-16">
                ...
                <div class="relative shadow-md sm:rounded-lg bg-white py-4">
                    ...
                    <div class="px-4 md:px-6 lg:px-8 mt-6 mb-6">
                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-lg p-6 shadow-sm">
                            ...
                            <p class="text-sm text-gray-600 mb-4">Upload an Excel file with all forgot password page setting translations for a specific language.</p>
                            ...
                            <div class="mt-4 pt-4 border-t border-blue-200">
                                <div class="flex items-center justify-between">
                                    ...
                                    <a :href="`${mixAdminApiUrl}download-forgot-password-page-setting-template?format=single_column`" target="_blank" class="inline-flex items-center px-4 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 transition-colors duration-200"><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>Download Template</a>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        async uploadExcelFile() {
            ...
            try {
                const response = await axios.post(`${process.env.MIX_ADMIN_API_URL}upload-forgot-password-page-setting-excel`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
                if (response?.data?.status === 'Success') {
                    ...
                    setTimeout(() => { this.getForgotPasswordPageSetting && this.getForgotPasswordPageSetting(); }, 1000);
                } else {
            ...
            axios
                ...
                .then((res) => {
                    if (res?.data?.status == "Success") {
                        ...
                        this.fetchForgotPasswordPageSetting();
                    }
                });
        },
        fetchForgotPasswordPageSetting() {
            axios
                .get(`${process.env.MIX_ADMIN_API_URL}get-forgot-password-page-setting`)
                .then((res) => {
                    if (res?.data?.status == "Success") {
                        let forgot_password_page_setting_detail =
                            res?.data?.data?.forgot_password_page_setting_detail || [];
                        forgot_password_page_setting_detail.map((setting) => {
                            this.handleInput(
                                setting?.name,
                                setting?.language,
                                "name"
        ...
        updatePageSetting() {
            ...
            axios
                .post(
                    `${process.env.MIX_ADMIN_API_URL}update-forgot-password-page-setting`,
                    this.form
                )
                .then((res) => {
                    if (res?.data?.status == "Success") {
