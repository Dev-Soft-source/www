        ...
        <section class="privacy-policy-section relative md:top-16">
            <main class="flex-1 max-h-full p-3 bg-gray-200 pb-2">
                <div class="relative shadow-md sm:rounded-lg bg-white py-4">
                    ...
                    >
                        <ExcelBulkImport
                            title="Cost Sharing page settings"
                            mode="all_languages"
                            :languages="languages"
                            download-endpoint="download-cost-sharing-page-setting-template"
                            upload-endpoint="upload-cost-sharing-page-setting-excel"
                            @success="fetchCostSharingPageSetting"
                        />
                        <div
...
export default {
    ...
    methods: {
            ...
            axios
                ...
                .then((res) => {
                    if (res?.data?.status == "Success") {
                        ...
                        this.fetchCostSharingPageSetting();
                    }
                });
        },
        fetchCostSharingPageSetting() {
            axios
                .get(`${process.env.MIX_ADMIN_API_URL}get-cost-sharing-page-setting`)
                .then((res) => {
                    if (res?.data?.status == "Success") {
                        let cost_sharing_page_setting_detail =
                            res?.data?.data?.cost_sharing_page_setting_detail || [];
                        cost_sharing_page_setting_detail.map((setting) => {
                            this.handleInput(
                                setting?.name,
                                setting?.language,
                                "name"
        ...
        updatePageSetting() {
            ...
            axios
                .post(
                    `${process.env.MIX_ADMIN_API_URL}update-cost-sharing-page-setting`,
                    this.form
                )
                .then((res) => {
                    if (res?.data?.status == "Success") {
