<template>
    <AppLayout>
        <section class="cancellation-section relative md:top-16">
                        ...
                        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                            <div class="flex items-center justify-between">
                                <h3 class="can-exp-h2 text-primary">
                                    Cancellation policy page settings
                                </h3>
                            </div>
                        </div>
                    </header>
                    ...
                    <div class="px-4 md:px-6 lg:px-8 mt-6 mb-6">
                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-lg p-6 shadow-sm">
                            ...
                            <p class="text-sm text-gray-600 mb-4">
                                Upload an Excel file with all cancellation page setting translations for a specific language. This will save or update all fields at once.
                            </p>
                            ...
                            <div class="mt-4 pt-4 border-t border-blue-200">
                                <div class="flex items-center justify-between">
                                    ...
                                    <a
                                        :href="`${mixAdminApiUrl}download-cancellation-page-setting-template?format=single_column`"
                                        target="_blank"
...
export default {
    ...
    methods: {
        ...
        fetchTermsOfUsePageSetting() {
            axios
                .get(`${process.env.MIX_ADMIN_API_URL}get-cancellation-page-setting`)
                .then((res) => {
                    if (res?.data?.status == "Success") {
                        let cancellation_page_setting_detail =
                            res?.data?.data?.cancellation_page_setting_detail || [];
                        cancellation_page_setting_detail.map((setting) => {
                            this.handleInput(
                                setting?.name,
                                setting?.language,
                                "name"
        ...
        updatePageSetting() {
            ...
            axios
                .post(
                    `${process.env.MIX_ADMIN_API_URL}update-cancellation-page-setting`,
                    this.form
                )
                .then((res) => {
                    if (res?.data?.status == "Success") {
        ...
        async uploadExcelFile() {
            ...
            try {
                const response = await axios.post(
                    `${process.env.MIX_ADMIN_API_URL}upload-cancellation-page-setting-excel`,
                    formData,
                    {
                        headers: {
