<template>
    <AppLayout>
        <section class="reset-password-section relative md:top-16">
            <main class="flex-1 max-h-full p-3 bg-gray-200 pb-2">
                <div class="relative shadow-md sm:rounded-lg bg-white py-4">
                    ...
                    <ExcelBulkImport
                        ...
                        mode="all_languages"
                        download-endpoint="download-reset-password-page-setting-template"
                        upload-endpoint="upload-reset-password-page-setting-excel"
                        @success="fetchResetPasswordPageSetting"
                    />
                    <form
...
export default {
    ...
    methods: {
            ...
            axios
                ...
                .then((res) => {
                    if (res?.data?.status == "Success") {
                        ...
                        this.fetchResetPasswordPageSetting();
                    }
                });
        },
        fetchResetPasswordPageSetting() {
            axios
                .get(`${process.env.MIX_ADMIN_API_URL}get-reset-password-page-setting`)
                .then((res) => {
                    if (res?.data?.status == "Success") {
                        let reset_password_page_setting_detail =
                            res?.data?.data?.reset_password_page_setting_detail || [];
                        reset_password_page_setting_detail.map((setting) => {
                            this.handleInput(
                                setting?.name,
                                setting?.language,
                                "name"
        ...
        updatePageSetting() {
            ...
            axios
                .post(
                    `${process.env.MIX_ADMIN_API_URL}update-reset-password-page-setting`,
                    this.form
                )
                .then((res) => {
                    if (res?.data?.status == "Success") {
