            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Closed Account messages</h3>
                    </div>
                </div>
                            ...
                            <table class="table overflow-x-auto table-auto w-full leading-normal text-base md:text-base lg:text-lg">
                                <thead class="text-white">
                                    <tr class="hidden md:table-row">
                                        ...
                                        <th
                                            class="sticky top-0 z-10 bg-primary backdrop-blur backdrop-filter py-3.5 pl-3 pr-3 text-left font-FuturaMdCnBT text-white lg:text-xl md:text-lg text-lg font-normal">
                                            Recommend ProximaRide to others?
                                        </th>
                                        <th
                                            class="sticky top-0 z-10 bg-primary backdrop-blur backdrop-filter py-3.5 pl-3 pr-3 text-left font-FuturaMdCnBT text-white lg:text-xl md:text-lg text-lg font-normal">
                                            how we can improve
...
export default {
    ...
    computed: {
        ...mapState({
            messages: (state) => state.closed_account_messages.messages,
            pagination: (state) => state.closed_account_messages.pagination,
            searchParam: (state) => state.closed_account_messages.searchParam,
            loading: (state) => state.closed_account_messages.loading,
        }),
        limit: {
            get() {
    ...
    methods: {
        fetchMessages(page_url) {
            this.$store.dispatch("closed_account_messages/fetchMessages", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("closed_account_messages/setLimit", value);
            this.$store.dispatch("closed_account_messages/fetchMessages");
        },
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("closed_account_messages/setSearchParam", this.quickSearch);
            this.$store.dispatch("closed_account_messages/fetchMessages");
        }, 500),
    },
    created() {
        this.$store.commit("closed_account_messages/setLimit", 100);
        this.$store.commit("closed_account_messages/setSortBy", "id");
        this.$store.commit("closed_account_messages/setSortType", "desc");
        this.$store.commit("closed_account_messages/setSearchParam", '');
        this.$store.dispatch("closed_account_messages/fetchMessages");
    },
    watch: {
