            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Banks</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.banks.create' }" class="block button-exp-fill w-full">
                            Add new bank
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-banks"
                            class="block  pl-10  w-full md:w-80 bg-white can-exp-input" placeholder="Search for banks"
                            v-model="quickSearch" />
                    </div>
                </div>
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="bank in banks" :key="bank.id"
                                        ...
                                        <td class="py-2 md:p-3  md:border-none">
                                            ...
                                            <div class="">{{ bank.bank_to_bank_fee }}</div>
                                        </td>
                                        <td class="py-2 md:p-3 md:border-none">
                                            <!-- <label class="text-gray-500 font-FuturaMdCnBT md:hidden text-xl"
                                                for="">Other bank fee</label> -->
                                            <div>{{ bank.other_bank_fee }}</div>
                                        </td>
                                        <td
                                            class="p-2 md:p-3 gap-1 md:gap-2 justify-center items-center absolute top-4 md:top-0 right-0 md:relative flex">
                                            <!-- <router-link
                                                :to="{ name: 'admin.banks.edit', params: { id: bank.id } }"
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.banks.edit', params: { id: bank.id } }).href"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click="fetchBanks(pagination.prev_page_url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="fetchBanks(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchBanks(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchBanks(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchBanks(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            pagination: (state) => state.banks.pagination,
            validationErros: (state) => state.banks.validationErros,
            searchParam: (state) => state.banks.searchParam,
            loading: (state) => state.banks.loading,
        }),
        limit: {
            get() {
                return this.$store.state.banks.limit;
            },
            set(value) {
                this.$store.commit('banks/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchBanks(page_url) {
            this.$store.dispatch("banks/fetchBanks", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("banks/setLimit", value);
            ...
            this.$swal
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("banks/deleteBank", {
                            id: bank.id,
                        })
                            .then((res) => {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("banks/setSearchParam", this.quickSearch);
            this.$store.dispatch("banks/fetchBanks");
        }, 500),
    },
    created() {
        this.$store.commit("banks/setLimit", 100);
        this.$store.commit("banks/setSortBy", "id");
        this.$store.commit("banks/setSortType", "desc");
        this.$store.commit("banks/setSearchParam", '');
        this.$store.dispatch("banks/fetchBanks");
    },
    watch: {
