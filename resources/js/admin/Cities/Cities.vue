            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Cities</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.cities.create' }" class="block button-exp-fill w-full">
                            Add new city
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-cities"
                            class="block  pl-10  w-full md:w-80 bg-white can-exp-input" placeholder="Search for cities"
                            v-model="quickSearch" />
                    </div>
                </div>
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="city in cities" :key="city.id"
                                        ...
                                        <td
                                            ...
                                            <!-- <router-link
                                                :to="{ name: 'admin.cities.edit', params: { id: city.id } }"
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.cities.edit', params: { id: city.id } }).href"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click="fetchCities(pagination.prev_page_url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="fetchCities(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchCities(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchCities(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchCities(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            pagination: (state) => state.cities.pagination,
            validationErros: (state) => state.cities.validationErros,
            searchParam: (state) => state.cities.searchParam,
            loading: (state) => state.cities.loading,
        }),
        limit: {
            get() {
                return this.$store.state.cities.limit;
            },
            set(value) {
                this.$store.commit('cities/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchCities(page_url) {
            this.$store.dispatch("cities/fetchCities", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("cities/setLimit", value);
            ...
            this.$swal
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("cities/deleteCity", {
                            id: city.id,
                        })
                            .then((res) => {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("cities/setSearchParam", this.quickSearch);
            this.$store.dispatch("cities/fetchCities");
        }, 500),
    },
    created() {
        this.$store.commit("cities/setLimit", 100);
        this.$store.commit("cities/setSortBy", "id");
        this.$store.commit("cities/setSortType", "desc");
        this.$store.commit("cities/setSearchParam", '');
        this.$store.dispatch("cities/fetchCities");
    },
    watch: {
