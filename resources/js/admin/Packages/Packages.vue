            ...
            <div class="px-4">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Packages</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.packages.create' }" class="block button-exp-fill w-full">
                            Add new package
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-packages"
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="pack in packages" :key="pack?.id"
                                        ...
                                        <td class="p-2 md:p-3 gap-2 justify-center items-center hidden md:flex">
                                            <!-- <router-link
                                                :to="{ name: 'admin.packages.edit', params: { id: pack?.id } }"
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.packages.edit', params: { id: pack?.id } }).href"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click="fetchPackages(pagination.prev_page_url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="fetchPackages(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchPackages(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchPackages(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchPackages(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            pagination: (state) => state.packages.pagination,
            validationErros: (state) => state.packages.validationErros,
            searchParam: (state) => state.packages.searchParam,
            loading: (state) => state.packages.loading,
        }),
        limit: {
            get() {
                return this.$store.state.packages.limit;
            },
            set(value) {
                this.$store.commit('packages/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchPackages(page_url) {
            this.$store.dispatch("packages/fetchPackages", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("packages/setLimit", value);
        ...
        deletePackage(id) {
            this.$swal
                .fire({
                    ...
                    customClass: {
                        confirmButton: 'text-base inline-flex items-center button-exp-fill',
                        cancelButton: 'text-base inline-flex items-center button-exp-fill',
                    },
                    didOpen: () => {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("packages/setSearchParam", this.quickSearch);
            this.$store.dispatch("packages/fetchPackages");
        }, 500),
    },
    created() {
