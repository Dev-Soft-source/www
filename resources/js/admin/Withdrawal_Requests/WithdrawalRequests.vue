    ...
    <AppLayout>
            ...
            <div class="px-4">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left" v-if="withdrawals">All withdrawals
                            <small v-if="withdrawals">({{ withdrawals.length }})</small>
                        </h3>
                    </div>
                </div>
                                    ...
                                    <tr v-for="withdrawal in withdrawals" :key="withdrawal.id"
                                        ...
                                        <td class="p-2 md:p-3 border-b md:border-none">
                                            ...
                                            <div v-if="withdrawal.bank_detail">
                                                <div v-if="withdrawal.bank_detail.payment_status === 'pending'">Paid for
                                                    verification: ${{ withdrawal.bank_detail.admin_verify_amount }} CAD
                                                </div>
                                                <div v-if="adminSetting && adminSetting.bank">
                                                    <span
                                                        v-if="adminSetting.bank_id === withdrawal.bank_detail.bank_id">Bank
                                                        to bank fee: ${{ adminSetting.bank.bank_to_bank_fee }}
                                                        CAD</span>
                                                    <span v-else>Other bank fee: {{ adminSetting.bank.other_bank_fee
                                                    }}</span>
                                                </div>
                                            </div>
                                        ...
                                        <td
                                            ...
                                            <div class="flex items-center space-x-2">
                                                <button class="inline-flex items-center button-exp-fill" type="button"
                                                    v-on:click="toggleModal(withdrawal)">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                <button class="inline-flex items-center button-exp-fill cursor-pointer"
                                                    type="button" v-on:click="toggleModal2(withdrawal)">
                                                    Transfer
                                                </button>
                                            </div>
                                        </td>
        ...
        <div v-if="showModal"
            ...
            <div class="relative my-6 mx-auto max-w-4xl w-full">
                <!--content-->
                <div
                    class=" animate__animated animate__fadeIn border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none w-full">
                    <!--header-->
                    <div
                        class="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t w-full">
                        <h5 class="modal-title"><b>{{ selectedWithdrawal.first_name }} {{ selectedWithdrawal.last_name
                        }}</b>
                        </h5>
                    </div>
                    <!--body-->
                    <div class="relative p-6 flex-auto h-80 overflow-auto">
                        <table class="table table-striped table-fixed w-full">
                            <tbody>
                                <tr>
                                    <td><b>Payout Summary:</b></td>
                                    <td>{{ selectedWithdrawal.method }}</td>
                                </tr>
                            </tbody>
                        </table>
                            ...
                            <table class="table table-striped table-fixed w-full">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ selectedWithdrawal.paypal_email }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                            ...
                            <table class="table table-striped table-fixed w-full">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ selectedWithdrawal.account_no }}</td>
                                    </tr>
                                    <tr>
                                        <td><b>Bank Name:</b></td>
                                    ...
                                    <tr>
                                        <td><b>Country:</b></td>
                                        <td>{{ selectedWithdrawal.country }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <table class="table table-striped table-fixed w-full">
                            <tbody v-for="booking in selectedWithdrawal.bookings" :key="booking.id">
                                <tr>
                                    <td><b>Booking user:</b></td>
                                    <td>{{ booking.passenger.first_name }}</td>
                                    <td><b>seats:</b></td>
                                    <td>{{ booking.seats }}</td>
                                    <td><b>Total amount:</b></td>
                                    <td>{{ Number(booking.fare).toFixed(2) }}</td>
                                    <td><b>Ride ID:</b></td>
                                    <td>{{ booking.ride_id }}</td>
                                    <td><b>From:</b></td>
                                    <td>{{ booking.ride.date }}</td>
                                    <td><b>To:</b></td>
                                    <td>{{ booking.ride.completed_date }}</td>
                                </tr>
                    ...
                    <div class="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                        <button
                            class="text-red-500 bg-transparent border border-solid border-red-500 hover:bg-red-500 hover:text-white active:bg-red-600 font-bold text-sm px-6 py-3 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
        ...
        <div v-if="showModal2"
            ...
            <div class="relative my-6 mx-auto w-1/2 max-w-6xl">
                ...
                <div
                    ...
                    <!--body-->
                    <div v-if="selectedWithdrawal.bank_detail && selectedWithdrawal.bank_detail.set_default === 'bank'"
                        class="relative p-6 flex-auto h-96 ove">
                        <table class="table table-striped table-fixed w-full">
                            <tbody>
                                <tr>
                                    <td><b>Account holder’s name:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.bank_title }}</td>
                                </tr>
                                <tr>
                                    <td><b>Account holder’s number:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.acc_no }}</td>
                                </tr>
                                <tr>
                                    <td><b>Account holder’s address:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.address }}</td>
                                </tr>
                                <tr>
                                    <td><b>Branch name:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.branch }}</td>
                                </tr>
                                <tr>
                                    <td><b>Branch number:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.branch_number }}</td>
                                </tr>
                                <tr>
                                    <td><b>Branch address:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.branch_address }}</td>
                                </tr>
                                <tr>
                                    <td><b>Bank name:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.bank.name }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div v-if="selectedWithdrawal.bank_detail && selectedWithdrawal.bank_detail.set_default === 'paypal'"
                        class="relative p-6 flex-auto">
                        <table class="table table-striped table-fixed w-full">
                            <tbody>
                                <tr>
                                    <td><b>PayPal email:</b></td>
                                    <td>{{ selectedWithdrawal.bank_detail.paypal_email }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                        ...
                        <button
                            class="inline-flex items-center ml-1 text-blue-500 bg-transparent border border-solid border-blue-500 hover:bg-blue-500 hover:text-white active:bg-red-600 button-exp-fill cursor-pointer"
