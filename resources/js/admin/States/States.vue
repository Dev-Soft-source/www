            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">States</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.states.create' }" class="block button-exp-fill w-full">
                            Add new state
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-states"
                            class="block  pl-10  w-full md:w-80 bg-white can-exp-input" placeholder="Search for states"
                            v-model="quickSearch" />
                    </div>
                </div>
                            ...
                            <table
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="state in states" :key="state.id"
                                        ...
                                        <td class="py-2 md:p-3 border-none relative">
                                        ...
                                        <td class="py-2 md:p-3 border-none">
                                        ...
                                        <td
                                            ...
                                            <!-- <router-link
                                                :to="{ name: 'admin.states.edit', params: { id: state.id } }"
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.states.edit', params: { id: state.id } }).href"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="deleteState(state)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click="fetchStates(pagination.prev_page_url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="fetchStates(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchStates(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchStates(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchStates(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            pagination: (state) => state.states.pagination,
            validationErros: (state) => state.states.validationErros,
            searchParam: (state) => state.states.searchParam,
            loading: (state) => state.states.loading,
        }),
        limit: {
            get() {
                return this.$store.state.states.limit;
            },
            set(value) {
                this.$store.commit('states/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchStates(page_url) {
            this.$store.dispatch("states/fetchStates", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("states/setLimit", value);
            ...
            this.$swal
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("states/deleteState", {
                            id: state.id,
                        })
                            .then((res) => {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("states/setSearchParam", this.quickSearch);
            this.$store.dispatch("states/fetchStates");
        }, 500),
        actionModal(id) {
            this.showModalId = id;
    ...
    created() {
        this.$store.commit("states/setLimit", 100);
        this.$store.commit("states/setSortBy", "id");
        this.$store.commit("states/setSortType", "desc");
        this.$store.commit("states/setSearchParam", '');
        this.$store.dispatch("states/fetchStates");
    },
    watch: {
