                ...
                <div class="max-w-full mx-auto px-4">
                    <div class="flex items-center justify-between">
                        <h3 class="can-exp-h2 text-primary">
                            {{ isFormEdit ? "Edit" : "Create" }} passenger reward point
                        </h3>
                        <router-link
                            :to="{ name: 'admin.passenger-reward-points.index' }"
...
export default {
    computed: {
        ...mapState({
            ...
            form: (state) => state.passengerRewardPointSettings.form,
            isFormEdit: (state) => state.passengerRewardPointSettings.isFormEdit,
        ...
        is_default: {
            ...
            set: function (val) {
                this.$store.commit("passengerRewardPointSettings/setForm", {
                    is_default: val,
                });
            },
        },
    ...
    methods: {
        handleInput(value, key) {
            this.$store.commit("passengerRewardPointSettings/setState", { key, value });
        },
        handleMultipleInput(key, value, language) {
            this.$store.commit("passengerRewardPointSettings/updateState", {
                value: value,
                id: language.id,
                key,
            });
        ...
        addUpdateForm() {
            this.$store
                .dispatch("passengerRewardPointSettings/addUpdateForm")
                .then(() =>
                    this.$router.push({ name: "admin.passenger-reward-points.index" })
                );
        },
        changeLanguageTab(language) {
            this.activeTab = language.id;
        },
        fetchPassengerRewardPoint() {
            if (this.$route.params.id) {
                let id = this.$route.params.id;

                this.$store.commit("passengerRewardPointSettings/setIsFormEdit", 1);
                this.$store
                    .dispatch("passengerRewardPointSettings/fetchPassengerRewardPoint", {
                        id: id,
                        url: `${process.env.MIX_ADMIN_API_URL}passenger-reward-points/${id}?withRewardPointSettingDetail=1`,
                    })
                    .then((res) => {
                        let keys = [
                            "page",
                        ];
                        this.$store.commit("passengerRewardPointSettings/setState", {
                            key: "id",
                            value: id,
                        });
    ...
    created() {
        this.$store.commit("passengerRewardPointSettings/resetForm");
        this.$store
            ...
            .then((res) => {
                ...
                this.fetchPassengerRewardPoint();
            });
    },
};
</script>
