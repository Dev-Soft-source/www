            ...
            <div class="px-4">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Passenger Reward Points</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.passenger-reward-points.create' }"
                            class="block button-exp-fill w-full">
                            Add new passenger reward point
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-banks"
                            ...
                            placeholder="Search for passenger reward point" v-model="quickSearch" />
                    </div>
                </div>
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr v-for="rewardPointSetting in rewardPointSettings" :key="rewardPointSetting.id"
                                        ...
                                        <td
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.passenger-reward-points.edit', params: { id: rewardPointSetting.id } }).href"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="deletePassengerRewardPoint(rewardPointSetting)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor"
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchPassengerRewardPoints(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchPassengerRewardPoints(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchPassengerRewardPoints(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    computed: {
        ...mapState({
            rewardPointSettings: (state) => state.passengerRewardPointSettings.rewardPointSettings,
            pagination: (state) => state.passengerRewardPointSettings.pagination,
            validationErros: (state) => state.passengerRewardPointSettings.validationErros,
            searchParam: (state) => state.passengerRewardPointSettings.searchParam,
            loading: (state) => state.passengerRewardPointSettings.loading,
        }),
        limit: {
            get() {
                return this.$store.state.passengerRewardPointSettings.limit;
            },
            set(value) {
                this.$store.commit('passengerRewardPointSettings/setLimit', value);
            }
        }
    },
    data() {
    ...
    methods: {
        fetchPassengerRewardPoints(page_url) {
            this.$store.dispatch("passengerRewardPointSettings/fetchPassengerRewardPoints", { url: page_url });
        },
        updateLimit(value) {
            this.$store.commit("passengerRewardPointSettings/setLimit", value);
            this.$store.dispatch("passengerRewardPointSettings/fetchPassengerRewardPoints");
        },
        deletePassengerRewardPoint(rewardPointSetting) {
            this.$swal
                .fire({
                    title: "Are you sure?",
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("passengerRewardPointSettings/deletePassengerRewardPoint", {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("passengerRewardPointSettings/setSearchParam", this.quickSearch);
            this.$store.dispatch("passengerRewardPointSettings/fetchPassengerRewardPoints");
        }, 500),
    },
    created() {
        this.$store.commit("passengerRewardPointSettings/setLimit", 100);
        this.$store.commit("passengerRewardPointSettings/setSortBy", "id");
        this.$store.commit("passengerRewardPointSettings/setSortType", "desc");
        this.$store.commit("passengerRewardPointSettings/setSearchParam", '');
        this.$store.dispatch("passengerRewardPointSettings/fetchPassengerRewardPoints");
    },
    watch: {
