            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h3 class="can-exp-h2 text-primary text-center sm:text-left">Articles</h3>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 flex justify-center">
                        <router-link :to="{ name: 'admin.news.create' }" class="block button-exp-fill w-full">
                            Add article
                        </router-link>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-4 py-4">
                    <div>
                        show
                        <select @input="updateLimit($event.target.value)" class="rounded-md px-3 pr-8 py-1">
                    ...
                    <div class="relative w-full md:w-auto">
                        ...
                        <input type="text" id="table-search-articles"
                        ...
                        <div class="bg-white shadow-lg hover:shadow-xl rounded-md overflow-x-auto">
                            <table
                                ...
                                <thead class="text-white">
                                    <tr class="hidden md:table-row">
                                        <th
                                            ...
                                            #ID</th>
                                        <!-- <th class="sticky top-0 z-10 bg-primary backdrop-blur backdrop-filter py-3.5 pl-3 pr-3 text-left font-FuturaMdCnBT text-white lg:text-xl md:text-lg text-lg font-normal">Image</th> -->
                                        <th
                                            class="sticky top-0 z-10 bg-primary backdrop-blur backdrop-filter py-3.5 pl-3 pr-3 text-left font-FuturaMdCnBT text-white lg:text-xl md:text-lg text-lg font-normal">
                                            Title</th>
                                        <th
                                ...
                                <tbody class="flex-1 text-gray-700 sm:flex-none">
                                    <tr class="border-t first:border-t-0 flex p-3 md:p-3  md:table-row flex-col w-full flex-wrap even:bg-gray-50 odd:bg-white"
                                        ...
                                        <!-- <td class="p-2 md:p-3 border-b md:border-none">
                                            <div class="">
                                                {{article?.article_detail && article?.article_detail[0] ? article?.image : ''}}
                                            </div>
                                        </td> -->
                                        <td class="p-2 md:p-3 border-b md:border-none">
                                            <div class="">
                                        ...
                                        <td class="p-2 md:p-3 gap-2 justify-center items-center hidden md:flex">
                                            <!-- <router-link :to="{ name: 'admin.news.edit', params: {id: article?.id} }" type="button" class="inline-flex items-center button-exp-fill">
                                            ...
                                            <a :href="$router.resolve({ name: 'admin.news.edit', params: { id: article?.id } }).href"
                            ...
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                              ...
                              <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination" v-if="pagination.next_page_url || pagination.prev_page_url">
                                    <a href="#" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50" v-bind:class="[{disabled: !pagination.prev_page_url}]" @click="fetchArticles(pagination.prev_page_url)">
                                    ...
                                    <a href="#" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50" v-bind:class="[{disabled: !pagination.next_page_url}]" @click.prevent="fetchArticles(pagination.next_page_url)">
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.prev_page_url && fetchArticles(pagination.prev_page_url)">
                                                Previous
                                            </a>
                                            <template v-for="(link, index) in pagination.links" :key="index">
                                                <a v-if="link.url && !link.label.includes('Previous') && !link.label.includes('Next')"
                                                    ...
                                                    @click.prevent="fetchArticles(link.url)">
                                            ...
                                            <a href="#"
                                                ...
                                                @click.prevent="pagination.next_page_url && fetchArticles(pagination.next_page_url)">
                                                Next
                                            </a>
                                        </nav>
                                    </div>
...
export default {
    ...
    created() {
        ...
        this.$store.commit("articles/setSortBy", "id");
        this.$store.commit("articles/setSortType", "desc");
        this.$store.dispatch("articles/fetchArticles");
    },
    watch: {
