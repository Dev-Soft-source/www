                        ...
                        @if (!empty($bookings) && $bookings->count() > 0)
                            @foreach ($bookings as $booking)
                                @if ($booking->ride)
                                    @php
                                        $ride = $booking->ride;
                                        $currencyCode = strtoupper((string) ($booking->currency ?? $ride->currency ?? 'USD'));
                                        $currencySymbol = $currencyMap[$currencyCode] ?? ($currencyCode . ' ');
                                    @endphp
                                    <x-px.ride-card
                                        :ride="$ride"
                                        :lang="optional($selectedLanguage)->abbreviation"
                                        :price-minor="$booking->segment_price_minor"
                                        :detail-query="[
                                    ...
                                    >
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                                            <p><strong>Booked section:</strong> {{ $booking->fromStop->label ?? 'N/A' }} -> {{ $booking->toStop->label ?? 'N/A' }}</p>
                                            <p><strong>Seats:</strong> {{ (int) $booking->seats }}</p>
                                            <p><strong>Total paid:</strong> {{ $currencySymbol }}{{ number_format(((int) $booking->total_price_minor) / 100, 2) }}</p>
                                            <p><strong>Status:</strong> {{ ucfirst((string) $booking->status) }}</p>
                                            <p><strong>Booked at:</strong> {{ optional($booking->booked_at)->format('F d, Y h:i A') ?? 'N/A' }}</p>
                                        </div>
                                    </x-px.ride-card>
                                @endif
                            @endforeach
