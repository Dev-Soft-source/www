            ...
            <form id="payment-form" method="POST" action="{{ route('my_cards.store') }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        @enderror
                        <div class="relative tooltip -bottom-4 group-hover:flex hidden client-error"
                            data-field="name_on_card">
                            <div role="tooltip"
                                class="relative tooltiptext -top-2 z-10 leading-none transition duration-150 ease-in-out shadow-lg p-2 flex bg-red-500 text-gray-600 w-full md:w-1/2 rounded">
                    ...
                    <div>
                        <label for="card_type">{{ $paymentSettingDetail->mobile_card_type_label ?? 'Card type' }} <span
                        ...
                        <select id="card_type" name="card_type"
                            ...
                            <option value="" {{ old('card_type') === '' ? 'selected' : '' }}>
                            ...
                            <option value="AmEx" {{ old('card_type') === 'AmEx' ? 'selected' : '' }}>
                                American Express
                            </option>
                            <option value="Dis" {{ old('card_type') === 'Dis' ? 'selected' : '' }}>
                                Discover
                            </option>
                            <option value="CUP" {{ old('card_type') === 'CUP' ? 'selected' : '' }}>
                                Union Pay
                            </option>
                            <option value="JC" {{ old('card_type') === 'JC' ? 'selected' : '' }}>
                                JCB International
                            </option>
                            <option value="DiC" {{ old('card_type') === 'DiC' ? 'selected' : '' }}>
                    ...
                    <div>
                        <label
                            for="exp_month">{{ $paymentSettingDetail->mobile_expiry_date_label ?? 'Expiry date' }}</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mt-2">
                    <div>
                        <label
                            for="street_address">{{ $paymentSettingDetail->mobile_street_name_label ?? 'Street number/name' }}</label>
                    ...
                    <div>
                        <label
                            for="house_apartment_number">{{ $paymentSettingDetail->mobile_house_number_label ?? 'House/apartment number (optional)' }}</label>
                        <input type="text" id="house_apartment_number" name="house_apartment_number"
                            value="{{ old('house_apartment_number') }}"
                            class="block mt-1 border p-1.5 w-full rounded text-base md:text-lg border-gray-300 focus:ring-none focus:outline-none focus:border-blue-600">
                    ...
                    <div>
                        <label
                            for="postal_code">{{ $paymentSettingDetail->mobile_postal_code_label ?? 'Postal code' }}</label>
                        <input type="text" maxlength="7" id="postal_code" name="postal_code"
                    ...
                    <div class="md:col-span-2">
                        <div class="flex items-center space-x-1.5 lg:space-x-3 mb-2 mr-2 lg:mr-2">
                            <input name="primary_card" type="checkbox" value="1"
                                {{ old('primary_card') == '1' ? 'checked' : '' }}
                                class="h-5 w-5 border-gray-300 bg-gray-200 cursor-pointer text-indigo-600 focus:ring-indigo-600">
                            <label for="primary_card"
                                class="block text-gray-900">{{ $paymentSettingDetail->mobile_primary_card_placeholder ?? 'Primary card' }}</label>
                        </div>
                    </div>

                    <div class="md:col-span-2 mt-4 flex justify-center">
                        <button type="submit"
                            class="button-exp-fill">{{ $paymentSettingDetail->save_button_text ?? 'Save' }}</button>
                    </div>
                </div>
            </form>
        </div>
...
@section('script')
    ...
    <script>
        ...
        var stripePlaceholders = {
            ...
            cardExpiry: @json(optional($paymentSettingDetail)->web_expiry_month_placeholder ??
                    (optional($paymentSettingDetail)->expiry_month_placeholder ?? 'MM / YY')),
            cardCvc: @json(optional($paymentSettingDetail)->security_code_palceholder ??
                    (optional($paymentSettingDetail)->security_code_placeholder ?? 'CVC'))
        };

        var cardNumberElement = elements.create('cardNumber', {
            placeholder: stripePlaceholders.cardNumber
        });
        cardNumberElement.mount('#card-number-element');

        var cardExpiryElement = elements.create('cardExpiry', {
            placeholder: stripePlaceholders.cardExpiry
        });
        cardExpiryElement.mount('#card-expiry-element');

        var cardCvcElement = elements.create('cardCvc', {
            placeholder: stripePlaceholders.cardCvc
        });
        ...
        function validateRequiredFields() {
            ...
            requiredFieldIds.forEach(function(id) {
                ...
                if (el && String(el.value).trim() === '') {
                    ...
                    if (!firstErrorContainer && container) {
            ...
            return {
                hasErrors: hasErrors,
                firstErrorEl: firstErrorContainer
            };
        }
        ...
        form.addEventListener('submit', function(event) {
            ...
            document.querySelectorAll('.showExpiryError, .showCardError, .showCvcError').forEach(function(el) {
            ...
            if (!isCardNumberComplete) {
                ...
                if (elMsg) {
                    elMsg.textContent = 'Card number is required.';
                }
                if (!stripeFirstErrorEl && els.length) {
                    stripeFirstErrorEl = els[0];
                }
            }
            if (!isExpiryComplete) {
                ...
                if (elMsg) {
                    elMsg.textContent = 'Expiry date is required.';
                }
                if (!stripeFirstErrorEl && els.length) {
                    stripeFirstErrorEl = els[0];
                }
            ...
            if (!isCvcComplete) {
                ...
                if (elMsg) {
                    elMsg.textContent = 'Security code is required.';
                }
                if (!stripeFirstErrorEl && els.length) {
                    stripeFirstErrorEl = els[0];
                }
            ...
            if (textValidation.firstErrorEl) {
                textValidation.firstErrorEl.scrollIntoView({
                    behavior: 'smooth',
                    block: 'center'
                });
            } else if (stripeFirstErrorEl) {
                stripeFirstErrorEl.scrollIntoView({
                    behavior: 'smooth',
                    block: 'center'
                });
            }
            ...
            stripe.createToken(cardNumberElement, {
                ...
                if (result.error) {
                    // debugger;
                    document.querySelectorAll('.showExpiryError, .showCardError, .showCvcError').forEach(
                        el => {
                            el.classList.add('hidden');
                        });
