    ...
    <div class="grid grid-cols-12 gap-4 md:container md:mx-auto my-6 md:my-10 xl:my-14 px-4 xl:px-0">
        ...
        <div class="bg-white border rounded p-4 border-gray-200 w-full col-span-12 lg:col-span-9 shadow">
            <div class="border-b pb-2 flex justify-between items-center">
                <h1 class="mb-0">
                    @isset($myVehiclePage->edit_main_heading)
                ...
                <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        <input type="text" name="make"
                            @if ($errors->count() > 0) value="{{ old('make') }}"
                        @else
                    ...
                    <div>
                        ...
                        <input type="text" name="model"
                            @if ($errors->count() > 0) value="{{ old('model') }}"
                        @else
                    ...
                    <div>
                        ...
                        <input type="text" name="color" maxlength="15"
                            @if ($errors->count() > 0) value="{{ old('color') }}"
                        @else
                    ...
                    <div>
                        ...
                        <input type="text" name="year" maxlength="4"
                            @if ($errors->count() > 0) value="{{ old('year') }}"
                        @else
                    ...
                    <div class="justify-between md:items-center gap-2">
                        <label for="primary_vehicle"
                            class="text-gray-900">{{ $myVehiclePage->set_primary_vehicle_label ?? 'Set as primary vehicle' }}
                            <span class="text-red-500">*</span></label>
                        <div class="mt-2 flex items-center gap-2">
                            <div class="flex items-center space-x-1.5 lg:space-x-3 mb-2 mr-2 lg:mr-2">
                    ...
                    <div class="md:col-span-2 flex justify-center">
                        <label for="dropzone-file" id="dropzone-file-label"
                            class="flex flex-col items-center justify-center w-1/2 h-auto border-2 border-gray-300 border-dashed rounded cursor-pointer bg-white hover:bg-gray-100">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6 p-4">
                                @if (session('uploaded_image'))
                                    <img id="profile-image" class="w-40 h-40 object-contain mb-4 cursor-pointer"
                                        src="{{ asset('car_images/' . session('uploaded_image')) }}"
                                        alt="Uploaded Image">
                                @elseif ($vehicle->image)
                                    <img id="profile-image" class="w-40 h-40 object-contain mb-4 cursor-pointer"
                                        src="{{ $vehicle->image }}" alt="Uploaded Image">
                                @else
                            ...
                            @elseif ($vehicle->image)
                                @php
                                    ...
                                    $imageName =
                                        $vehicle->getAttributes()['image'] ??
                                        basename(parse_url($vehicle->image, PHP_URL_PATH));
                                @endphp
                    ...
                    <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row justify-center md:items-center mt-1 gap-2 md:col-span-2">
                        <div id="hide-buttons" class="hidden flex items-center gap-2">
                            <label for="edit-file"
                                ...
                                <input id="edit-file" name="image" type="file" onchange="previewImage(this)" accept="image/*"
                                    class="hidden" />
                            </label>
                            <button class="bg-primary p-1 rounded cursor-pointer" type="button"
                                onclick="toggleModalCard('card-modal-1')">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="2" stroke="currentColor" class="w-6 h-6 text-white bg-primary">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="mt-4 flex justify-center md:col-span-2">
                        <button type="submit" id="update-vehicle-btn"
                            class="button-exp-fill w-32 opacity-50 cursor-not-allowed" disabled>
                            @isset($myVehiclePage->update_vehicle_button_text)
    ...
    <div class="hidden overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center"
        ...
        <div class="relative h-screen my-6 mx-auto flex items-center justify-center w-full" id="card-modal-1-backdrop-1">
            <div
                ...
                <button onclick="toggleModalCard('card-modal-1')"
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    ...
                    <div class="text-center sm:ml-4 sm:mt-0 sm:text-left">
                        <div class="mt-2 w-full">
                            <p class="text-lg text-center text-black">{{ $myVehiclePage->delete_photo_message ?? 'Are you sure you want to remove your car photo?' }}</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0 w-full"
                ...
                <div
                    ...
                    <button onclick="toggleModalCard('modal')"
                        class="absolute top-2 right-2 p-1 rounded-full hover:bg-gray-100">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        ...
                        <div class="mt-2 w-full">
                            <p class="can-exp-p text-center">
                                {{ $myVehiclePage->delete_photo_message ?? 'Are you sure you want to remove your car photo?' }}
                            </p>
                        </div>
                    </div>
                    <div class="px-4 pb-6 pt-4 sm:flex sm:flex-row-reverse sm:px-6 justify-center">
                        <button
                            ...
                            onclick="toggleModalCard('modal')">{{ $successMessage->no_go_back_button_text ?? 'No, go back' }}</button>
                        <button
                            class="yes-button inline-flex w-full justinline-flex justify-center rounded bg-blue-500 px-3 py-2 font-FuturaMdCnBT text-lg font-medium text-white hover:text-white hover:shadow-lg shadow-sm hover:bg-blue-400 sm:ml-3 sm:w-auto">{{ $successMessage->yes_remove_it_button_text ?? 'Yes, remove it' }}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function checkFormChanges() {
                ...
                $('form input, form select, form textarea').each(function() {
                    var name = $(this).attr('name');
                    if (name && name !== '_token' && name !== '_method' && name !== 'image' && name !==
                        'existing_image') {
                        var currentValue = '';
                        if ($(this).is(':checkbox') || $(this).is(':radio')) {
        ...
        document.getElementById('remove-photo').addEventListener('click', function(event) {
            ...
            if (profileImage.src && profileImage.src !== '') {
                ...
                const fileInput2 = document.getElementById('existing_image');
                if (fileInput2) {
                    fileInput2.remove(); // removes the element from the DOM
                }
