            ...
            <table style="width: 100%; padding: 10px 16px;" cellpadding="0" cellspacing="0" role="none">
                <tr>
                    <td>
                        ...
                        <p style="font-weight: 400;color: #000;">A new email address has just been added to your ProximaRide profile. If it was you who did this, disregard this message.</p>
                        <p style="font-weight: 400;color: #000;">If not, contact us immediately from your app and the old (original) email.</p>
                    </td>
                </tr>
                <tr>
                    <th>
                ...
                <tr>
                    <td align="center">
                        @if(isset($data['token']) && $data['token'])
                            <p style="text-align: center;"><a style="border-radius: 4px; background-color: #2563eb; padding: 12px 24px; font-size: 16px; font-weight: bold; color: #ffffff; text-decoration: none; display: inline-block;margin: 0 auto;"
                                href="{{ route('emailVerify', ['token' => $data['token'], 'email' => $data['email']]) }}{{ isset($data['app']) && $data['app'] ? '?app=true' : '' }}"
                                target="_blank">Verify email</a></p>
                        {{-- <br> --}}
                            <p style="font-weight: 400;color: #000;">If your browser does not permit you to click the link
                                above, copy and paste this URL into your web browser's address bar <br> <a
                                    href="{{ route('emailVerify', ['token' => $data['token'], 'email' => $data['email']]) }}{{ isset($data['app']) && $data['app'] ? '?app=true' : '' }}"
                                    target="_blank"
                                    style="margin: 10px auto;">{{ env('APP_URL') }}/email-verify/{{ $data['token'] }}/{{ $data['email'] }}{{ isset($data['app']) && $data['app'] ? '?app=true' : '' }}</a>
                            </p>
                        @endif
                    </td>
                </tr>
