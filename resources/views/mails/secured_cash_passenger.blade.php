        ...
        <tr>
          <td style="width: 600px; max-width: 100%;background:#fff; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);">
            ...
            <table style="width: 100%; padding: 10px 16px;" cellpadding="0" cellspacing="0" role="none">
                <tr>
                    <td>
                        <h1 style="font-weight: 700;color: #000;">Hi {{ $data['passenger_first_name'] }},</h1>
                        <p style="font-weight: 400;color: #000;">Thank you for providing the Secured-cash payment code to the driver. The code was entered successfully and your booking price has been refunded to you. Since this was an authorization and not a purchase, the funds must be available in your card or payment method instantly.</p>
                        <p style="font-weight: 400;color: #000;">Now, please pay your driver in cash. Pay the booking price only, not the booking fee.</p>

                        <p style="font-weight: 700;color: #000;">Booking details:</p>
                        <p style="font-weight: 400;color: #000;">Seats booked: {{ numberToWords($data['seats_booked']) }}</p>
                        <p style="font-weight: 400;color: #000;">Booking price (total): ${{ $data['booking_price'] }}</p>

                        <p style="font-weight: 700;color: #000;">Trip details:</p>
                        <p style="font-weight: 400;color: #000;">From: {{ $data['from'] }}</p>
                        <p style="font-weight: 400;color: #000;">To: {{ $data['to'] }}</p>
                        <p style="font-weight: 400;color: #000;">On: {{ $data['on'] }}</p>
                        <p style="font-weight: 400;color: #000;">At: {{ \Carbon\Carbon::parse($data['at'])->format('h:i A') }}</p>

                        <p style="font-weight: 400;color: #000;">Driver name: {{ $data['driver_first_name'] }}</p>
                        <p style="font-weight: 400;color: #000;">Driver phone number: {{ $data['driver_phone'] }}</p>
                        <p style="font-weight: 400;color: #000;">Driver email: {{ $data['passenger_email'] }}</p>
                    </td>
                </tr>
                <tr>
                    <th>
                        <p style="color: #000; font-weight: 400;margin-top:12px;margin-bottom: 0;">Thank you and have a safe trip</p>
