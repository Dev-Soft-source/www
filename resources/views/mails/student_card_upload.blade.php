        ...
        <tr>
          <td style="width: 600px; max-width: 100%;background:#fff; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);">
            ...
            <table style="width: 100%; padding: 10px 16px;" cellpadding="0" cellspacing="0" role="none">
                <tr>
                    <td>
                        <h1 style="font-weight: 700;color: #000;">🎓 Student Card Approval Required</h1>
                        <p style="font-weight: 400;color: #000;">
                            Hello {{ $data['username'] }},
                        </p>
                        <p style="font-weight: 400;color: #000;">
                            A user has uploaded a new student card for verification. Please review and approve/reject it from the admin dashboard.
                        </p>

                        <div style="background-color: #f3f4f6; padding: 16px; border-radius: 8px; margin: 16px 0;">
                            <h3 style="margin-top: 0; color: #000;">Student Details:</h3>
                            <p style="font-weight: 400;color: #000; margin: 8px 0;"><strong>Name:</strong> {{ $data['first_name'] }} {{ $data['last_name'] }}</p>
                            <p style="font-weight: 400;color: #000; margin: 8px 0;"><strong>Email:</strong> {{ $data['email'] }}</p>
                            <p style="font-weight: 400;color: #000; margin: 8px 0;"><strong>Country:</strong> {{ $data['country'] }}</p>
                            @if(isset($data['upload_date']))
                            <p style="font-weight: 400;color: #000; margin: 8px 0;"><strong>Upload Date:</strong> {{ $data['upload_date'] }}</p>
                            @endif
                            @if(isset($data['expiry_date']))
                            <p style="font-weight: 400;color: #000; margin: 8px 0;"><strong>Card Expiry Date:</strong> {{ $data['expiry_date'] }}</p>
                            @endif
                        </div>

                        <p style="font-weight: 400;color: #000; margin-top: 16px;">
                            ⚠️ <strong>Action Required:</strong> Please log in to your admin dashboard to review the uploaded student card and approve or reject it.
                        </p>
                    </td>
                </tr>
                <tr>
