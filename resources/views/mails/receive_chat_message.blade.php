        ...
        <tr>
          <td style="width: 600px; max-width: 100%;background:#fff; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);">
            ...
            <table style="width: 100%; padding: 10px 16px;" cellpadding="0" cellspacing="0" role="none">
                <tr>
                    <td>
                        <h1 style="font-weight: 700;color: #000;">{{ $greeting_message ?? 'Hi' }} {{ $data['receiverFirstName'] }}, </h1>
                        {{-- <p style="font-weight: 400;color: #000;">You have a new message from {{ $data['senderFirstName'] }}. Log in to read it</p> --}}
                        <p style="font-weight: 400;color: #000;">
                            @if ($data['type'] == 'driver')
                                You have a new message from your driver {{ $data['senderFirstName'] }}. Log in to read it
                            @else
                                You have a new message from {{ $data['senderFirstName'] }}. Log in to read it
                            @endif
                        </p>
                        <!-- {{ $data['senderLastName'] }} -->
                        {{-- <p style="font-weight: 400;color: #000;">Log in to read it</p> --}}
                        {{-- <p style="font-weight: 400;color: #000;">Here it is:  {{ $data['message'] }}</p> --}}


                        @if(isset($data['from']) && isset($data['to']))
                        <p style="font-weight: 400;color: #000;"><b>Ride details: </b></p><br>
                        ...
                        {{-- <p style="font-weight: 400;color: #000;"><b>Booking details: </b></p><br>
                        <p style="font-weight: 400;color: #000;">Seats booked: {{ numberToWords($data['seats']) }}</p>
                        <p style="font-weight: 400;color: #000;">Booking price (total): ${{ $data['price'] }}</p> --}}



                        @if ($data['type'] == 'passenger')
                            @if (!empty($data['isBooked']) && $data['isBooked'])
                                <p style="font-weight: 400;color: #000;">
                                    Thank you for reaching out to your passenger.
                                </p>
                            @else
                                <p style="font-weight: 400;color: #000;">
                                    {{ $data['senderFirstName'] }} has not booked seats on your ride yet. So, for your and their protection, do not offer, or accept offers to, share contact information, or make payments outside the platform.
                                </p>
                            @endif
                        @elseif ($data['type'] == 'driver')
                            <p style="font-weight: 400;color: #000;">
                                It is important to respond to all your messages and answer your driver’s questions in timely manner
                            </p>
                    @endif
