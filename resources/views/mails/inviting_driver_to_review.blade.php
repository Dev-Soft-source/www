            ...
            <table style="width: 100%; padding: 10px 16px;" cellpadding="0" cellspacing="0" role="none">
                <tr>
                    <td>
                        ...
                        <p style="font-weight: 400;color: #000;">Thank you for using ProximaRide! Please take a minute to tell us about your experience by clicking the link below:</p>
                        @foreach($data['getBookings'] as $booking)
                            <p style="font-weight: 400;color: #000;text-align: center;">
                                {{-- {{ $booking->passenger->first_name }} {{ $booking->passenger->last_name }} --}}
                                <a style="background-color: #2563eb; color: white; text-decoration: none; padding: 1% 2%; border-radius: 4px" href="{{ route('review_passenger', ['lang' => $data['abbreviation'], 'id' => $booking->uuid]) }}"
                                target="_blank">Give us your feedback</a>
                            </p>
                        @endforeach
                    </td>
                </tr>
