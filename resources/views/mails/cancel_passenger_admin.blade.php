        ...
        <tr>
          <td style="width: 600px; max-width: 100%;background:#fff; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);">
            ...
            <table style="width: 100%; padding: 10px 16px;" cellpadding="0" cellspacing="0" role="none">
                <tr>
                    <td>
                        <h1 style="font-weight: 700;color: #000;">Hello {{ $data['admin_username'] }},</h1>
                        <p style="font-weight: 400;color: #000;">The driver {{ $data['driver_name'] }} has removed the passenger {{ $data['passenger_name'] }} from the ride from {{ $data['departure'] }} to {{ $data['destination'] }} on {{ \Carbon\Carbon::parse($data['date'])->format('F d, Y') }}</p>
                        <p style="font-weight: 400;color: #000;">Driver’s explanation: {{ $data['message'] }}</p>
                    </td>
                </tr>
                <tr>
                    <th>
