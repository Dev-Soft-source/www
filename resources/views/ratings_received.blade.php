        ...
        <div class="bg-white border rounded p-4 border-gray-200 w-full col-span-12 lg:col-span-9 shadow">
            ...
            <div class="space-y-4 pb-2 max-h-[53rem] overflow-y-auto pr-2">
                ...
                @foreach ($allRatings as $index => $rating)
                    @if ($rating->from)
                        <div
                            class="relative even:bg-gray-100 odd:bg-white {{ $index >= $displayLimit ? 'hidden-rating hidden' : '' }}">

                            <a
                                @if ($rating->type === '2') href="{{ route('review.index', ['lang' => $selectedLanguage->abbreviation, 'id' => $rating->id]) }}"
                            @else
                                ...
                                <div class="rounded border border-gray-100 shadow p-4 md:p-6">
                                    <div class='flex items-start md:justify-center space-x-4'>
                                        <div class="">
                                            <div
                                                class="w-16 h-16 md:w-28 md:h-28 bg-gray-50 border rounded-full mx-auto overflow-hidden">
                                        ...
                                        <div class="flex-auto w-[70%]">
                                            <div class="flex justify-between">
                                                <div class="">
                                                    <h6 class='card-title leading-7 m-0'>{{ $rating->from->first_name }}
                                                    </h6>
                                            ...
                                            @if ($rating->replies)
                                                @foreach ($rating->replies as $reply)
                                                    <p class="text-base text-black">
                                                        <span
                                                            class="text-primary">{{ $reviewSettingPage->replied_label ?? 'Replied' }}:</span>
                                    ...
                                    <div class="flex justify-end mt-2">
                                        @if ($rating->reply_deadline)
                                            <a href="{{ route('review_reply', ['lang' => $selectedLanguage->abbreviation, 'id' => $rating->id]) }}"
                                                class='button-exp-fill h-full'>{{ $reviewSettingPage->reply_label ?? 'Reply' }}</a>
                                        @endif
                                    </div>
                                </div>
                            </a>
