...
@section('content')
    ...
    <div class="mx-auto max-w-2xl lg:max-w-4xl">
        <div class="flex min-h-full flex-col justify-center my-4 px-4 sm:px-6 lg:px-8">
            <div class="bg-white border border-gray-100 p-4 shadow rounded-md sm:px-10">
                ...
                <div class="sm:mx-auto sm:w-full sm:max-w-xl md:min-w-[26rem]">
                    ...
                    <form id="login-form" class="space-y-6" method="POST"
                        action="{{ route('login', ['lang' => $selectedLanguage->abbreviation ?? 'en']) }}">
                        @csrf
                        @if (session('error'))
                            <div id="error-modal" class="relative z-50" aria-labelledby="modal-title" role="dialog"
                                aria-modal="true" onclick="closeModal('error-modal')" style="display: block;">
                        ...
                        @if (session('message'))
                            <div id="message-modal" class="relative z-50" aria-labelledby="modal-title" role="dialog"
                                aria-modal="true">
                                    ...
                                    <div
                                        ...
                                        <div
                                            ...
                                            <div class="px-4 pb-6 pt-4 flex items-center justify-center">
                                                @php
                                                    $isApp =
                                                        request()->query('app') === 'true' || session('is_app_request');
                                                @endphp
                                                @if ($isApp)
                                                    <button
                        ...
                        <div class="mt-2">
                            <x-form.input label="{{ $loginPage->email_label }}" name="email" type="text"
                                placeholder="{{ $loginPage->email_placeholder }}" class=""
                                title="Please enter a valid email address, such as name@example.com" />
                        </div>

                        <div class="mt-2">
                            <x-form.input label="{{ $loginPage->password_label }}" name="password" type="password"
                                class="" />
                        </div>
                        <div class="mt-2 flex justify-between items-center w-full">
                            <div class="flex items-center gap-2">
                                ...
                                <label for="remember"
                                    class="form-check-label mt-1 text-md md:text-lg cursor-pointer">{{ $loginPage->remember_me_text ?? 'Remember me' }}</label>
                            </div>
                            <div class="flex items-center justify-end">
                                <div class="text-sm">
                                    <a tabindex="-1"
                                        href="{{ route('forgot.password', ['lang' => $selectedLanguage->abbreviation]) }}"
                                        class="font-lg text-blue-600 font-FuturaMdCnBT hover:text-indigo-500 mt-1 text-md md:text-lg">
                ...
                <div class="flex items-center justify-center space-x-1.5 md:space-x-4 my-4">
                    <a href="{{ route('signup.redirectToProvider', ['lang' => $selectedLanguage->abbreviation, 'provider' => 'facebook', 'intent' => 'login']) }}"
                    ...
                    <a href="{{ route('signup.redirectToProvider', ['lang' => $selectedLanguage->abbreviation, 'provider' => 'google', 'intent' => 'login']) }}"
                        class="w-12 md:w-14 h-12 md:h-14 rounded border flex justify-center items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" x="0px" y="0px" width="100"
                            height="100" viewBox="0 0 48 48">
                            <path fill="#FFC107"
            ...
            <div class="rounded-md p-3 mt-6 mb-10 shadow bg-white">
                <div class="flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" width="16"
                        height="16" fill="#2563eb" class="bi bi-shield-shaded w-5 h-5" viewBox="0 0 16 16">
                    ...
                    <p class="can-exp-p text-2xl text-blue-600 font-FuturaMdCnBT">
                        {{ $loginPage->protect_account_heading ?? 'Protect your account' }}</p>
                </div>
                <div class="mt-2 text-left mb-2">
                    <p class="text-left">
                        {{ $loginPage->protect_account_text ?? 'Whenever you sign in to the ProximaRide website, ensure that the web address in the browser starts with:' }}
                    </p>
                </div>
            </div>
        </div>
