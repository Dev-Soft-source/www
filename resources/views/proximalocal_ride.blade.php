...
@section('style')
    ...
    <style>
        ...
        .proximalocal-ride-faq__item:last-child {
            margin-bottom: 0;
        }
                ...
                <div class="mt-6 space-y-4">
                    ...
                    @elseif ($paginatedRides && $paginatedRides->count() > 0)
                            ...
                            @foreach ($paginatedRides->filter(fn($ride) => $ride->type === 'ride') as $ride)
                                ...
                                <div class="relative even:bg-white odd:bg-gray-200 space-y-4 rounded-lg">
                                    ...
                                    @elseif (auth()->user() && in_array('I only want passengers with reviews', explode('=', $ride->features)))
                                        @if ($filteredRatings->count() === 0) href="javascript:void(0);" onclick="toggleModal1('modal-id1', proximalLocalModalMessages.passenger_with_review_message)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @elseif (auth()->user() &&
                                            in_array($findRidePage->ride_features_option15->features_setting_id ?? null, explode('=', $ride->features)))
                                        @if ($totalAverage < 3) href="javascript:void(0);" onclick="toggleModal1('modal-id1', proximalLocalModalMessages.star3_passenger_message)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @elseif (auth()->user() &&
                                            in_array($findRidePage->ride_features_option14->features_setting_id ?? null, explode('=', $ride->features)))
                                        @if ($totalAverage < 4) href="javascript:void(0);" onclick="toggleModal1('modal-id1', proximalLocalModalMessages.star4_passenger_message)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @elseif (auth()->user() &&
                                            in_array($findRidePage->ride_features_option13->features_setting_id ?? null, explode('=', $ride->features)))
                                        @if ($totalAverage < 4.5) href="javascript:void(0);" onclick="toggleModal1('modal-id1', proximalLocalModalMessages.star5_passenger_driver)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @else
                                        href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}"
                                                    ...
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        @if (isset($firm_cancellation_discount) &&
                                                            ...
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                                ...
                                                                class="bi bi-info-circle-fill text-black cursor-help inline-flex"
                                                                viewBox="0 0 16 16"
                        ...
                        @if ($paginatedRides->filter(fn($ride) => $ride->type === 'otherRide')->count() > 0)
                            <div class="border-b border-gray-400 flex flex-col items-center justify-center pt-6">
                                @if ($paginatedRides->filter(fn($ride) => $ride->type === 'ride')->count() <= 0)
                                    <h3 class="text-primary">Sorry, we couldn't find any ProximaLocal Rides matching your
                                        search.</h3>
                                @endif
                                <h3 class="text-primary">More rides from {{ $request->from }} to {{ $request->to }}.
                                    Important: these are NOT ProximaLocal Rides</h3>
                            </div>
                                        ...
                                        <div class="rounded-lg shadow-3xl border-[3px] border-solid @if ($ride->status === '2') border-red-500 @elseif(isset($findRidePage->ride_features_option1->features_setting_id) &&
                                            ...
                                            <div class="flex items-center justify-between pb-0 p-4">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    @if (in_array($findRidePage->ride_features_option1->features_setting_id ?? null, explode('=', $ride->features)))
                                                        <button type="button"
                                                            onclick="event.preventDefault(); event.stopPropagation(); toggleModal1('modal-id2', '{{ $postRidePage->features_option1_tooltip }}', '{{ $findRidePage->ride_features_option1->name ?? $findRidePage->ride_features_option1->label }}')">
                                                            <img class="ml-2 w-12 h-12"
                                                                src="{{ asset('home_page_icons/' . $findRidePage->ride_features_option1->icon) }}"
                                                                alt="">
                                                        </button>
                                                    @endif
                                                    @if (in_array($findRidePage->ride_features_option2->features_setting_id ?? null, explode('=', $ride->features)))
                                                        <button type="button"
                                                            onclick="event.preventDefault(); event.stopPropagation(); toggleModal1('modal-id2', '{{ $postRidePage->features_option2_tooltip }}', '{{ $findRidePage->ride_features_option2->name ?? $findRidePage->ride_features_option2->label }}')">
                                                            <img class="ml-2 w-12 h-12"
                                                                src="{{ asset('home_page_icons/' . $findRidePage->ride_features_option2->icon) }}"
                                                                alt="">
                                                        </button>
                    ...
                    @elseif ($paginatedRides && $paginatedRides->count() == 0)
                        <div class="flex flex-col items-center justify-center">
                            <h3 class="text-primary">Sorry, we couldn't find any ProximaLocal Rides matching your search.
                            </h3>
                        </div>
                    @endif
                </div>
...
@section('script')
    ...
    <script>
        ...
        window.initProximalocalRidePlaces = function() {
            ...
            fromAutocompleteProximalocalRide = new google.maps.places.Autocomplete(fromInput, {
            ...
            toAutocompleteProximalocalRide = new google.maps.places.Autocomplete(toInput, {
                componentRestrictions: {
                    country: 'ca'
                },
                types: ['(cities)'],
            ...
            fromAutocompleteProximalocalRide.addListener('place_changed', function() {
                ...
                if (place.address_components && place.place_id) {
                    ...
                    isSelectingFromDropdownProximalocalRide = true;
                    ...
                    setTimeout(function() {
                        isSettingPlaceValueProximalocalRide = false;
                        isSelectingFromDropdownProximalocalRide = false;
                    }, 100);
                }
            });
            ...
            fromInput.addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    ...
                    resolveTypedCityValueProximalocalRide(this.value, 'from').then(function() {
            ...
            toInput.addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    event.preventDefault();
                    resolveTypedCityValueProximalocalRide(this.value, 'to').then(function() {
                        navigateToSearchRoute1();
                    });
                }
            });
            ...
            fromInput.addEventListener('blur', function() {
                ...
                setTimeout(async function() {
                    if (isSettingPlaceValueProximalocalRide ||
                        isSelectingFromDropdownProximalocalRide) return;
                        ...
                        if (currentValue !== '' && fromInputError) {
                            ...
                            if (tooltipError) {
                                tooltipError.textContent = currentValue === '' ?
                                    errorFromRequiredProximalocalRide :
                                    errorCityMissingProximalocalRide;
                            }
                ...
                setTimeout(async function() {
                    ...
                    if (currentValue === '' || !selectedToPlace || currentValue !== selectedToPlace
                        ...
                        if (currentValue !== '' && toInputError) {
                            ...
                            if (tooltipError) {
                                tooltipError.textContent = currentValue === '' ?
                                    errorToRequiredProximalocalRide :
                                    errorCityMissingProximalocalRide;
                            }
        ...
        function resolveTypedCityValueProximalocalRide(rawValue, target) {
            ...
            return new Promise(function(resolve) {
                geocoderProximalocalRide.geocode({
                    address: value,
                    componentRestrictions: {
                        country: 'CA'
                    }
        ...
        function navigateToSearchRoute() {
            ...
            if (fromInvalid || toInvalid) {
                if (fromInvalid && fromInputError) {
                    ...
                    if (te) te.textContent = !fromValue.trim() ? errorFromRequiredProximalocalRide :
                        errorCityMissingProximalocalRide;
                    fromInputError.classList.remove('hidden');
                }
                if (toInvalid && toInputError) {
                    var te2 = toInputError.querySelector('.tooltip-error');
                    if (te2) te2.textContent = !toValue.trim() ? errorToRequiredProximalocalRide :
                        errorCityMissingProximalocalRide;
                    toInputError.classList.remove('hidden');
                }
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ env('GOOGLE_API_KEY') }}&libraries=places&callback=initProximalocalRidePlaces"
        async defer></script>
@endsection
