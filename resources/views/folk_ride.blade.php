                    ...
                    @elseif ($paginatedRides && $paginatedRides->count() > 0)
                        @if ($paginatedRides->filter(fn($ride) => $ride->type === 'ride')->count() > 0)
                            <div class="flex flex-col items-center justify-center border-b border-gray-400">
                                <h3 class="text-primary">
                                    {{ $findRidePage->search_results_extra_care_ride_label ?? 'Search results for Folk Ride' }}
                                </h3>
                            </div>
                                ...
                                <div class="relative even:bg-white odd:bg-gray-200 space-y-4 rounded-lg">
                                    ...
                                    <a @if ($ride->status === '2') href=""
                                        @elseif (auth()->user() &&
                                                ...
                                                auth()->user()->address != '')
                                            href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgFemale)"
                                        @elseif (auth()->user() &&
                                            ...
                                            @if ($filteredRatings->count() === 0 && auth()->user()->address == '')
                                                href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgCompleteAddress)"
                                            ...
                                            @elseif ($totalAverage < 4.5 && auth()->user()->address !== '')
                                                href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar5)"
                                            @elseif ($totalAverage < 4.5 && auth()->user()->address == '')
                                                href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar5CompleteAddress)"
                                            @else
                                    ...
                                    @elseif (auth()->user() && in_array('I only want passengers with reviews', explode('=', $ride->features)))
                                        @if ($filteredRatings->count() === 0) href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgPassengerWithReviews)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @elseif (auth()->user() &&
                                            in_array($findRidePage->ride_features_option15->features_setting_id ?? null, explode('=', $ride->features)))
                                        @if ($totalAverage < 3) href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar3)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @elseif (auth()->user() &&
                                            in_array($findRidePage->ride_features_option14->features_setting_id ?? null, explode('=', $ride->features)))
                                        @if ($totalAverage < 4) href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar4)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @elseif (auth()->user() &&
                                            in_array($findRidePage->ride_features_option13->features_setting_id ?? null, explode('=', $ride->features)))
                                        @if ($totalAverage < 4.5) href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar5Driver)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}" @endif
                                    @else
                                        href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}"
                                                ...
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    @if (in_array($findRidePage->ride_features_option2->features_setting_id ?? null, explode('=', $ride->features)))
                                                        <span class="ml-2 inline-block cursor-help"
                                                            ...
                                                            title="{{ $postRidePage->features_option2_tooltip ?? '' }}">
                                                            <img class="w-12 h-12"
                                                    ...
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        @if (isset($firm_cancellation_discount) &&
                                                            ...
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                                ...
                                                                viewBox="0 0 16 16"
                                                                data-tippy-content="{!! nl2br(
                                                                    $findRidePage->firm_cancellation_tooltip ??
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex flex-nowrap items-center gap-1 md:gap-2">
                                                        @if ($ride->payment_method == ($findRidePage->payment_methods_option1->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                src="{{ asset('home_page_icons/' . $findRidePage->payment_methods_option1->icon) }}"
                                                                alt=""
                                                                data-tippy-content="{{ $postRidePage->payment_methods_option1_tooltip }}"
                                                                title="{{ $postRidePage->payment_methods_option1_tooltip }}">
                                                        @elseif ($ride->payment_method == ($findRidePage->payment_methods_option3->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->payment_methods_option2_tooltip }}">
                                                        @elseif ($ride->payment_method == ($findRidePage->payment_methods_option4->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->payment_methods_option3_tooltip }}">
                                                        @endif
                                                        @if ($ride->smoke == ($findRidePage->smoking_option2->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->smoking_option2_tooltip }}">
                                                        @endif
                                                        @if ($ride->animal_friendly == ($findRidePage->pets_allowed_option2->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->animals_option2_tooltip }}">
                                                        @elseif ($ride->animal_friendly == ($findRidePage->pets_allowed_option3->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->animals_option3_tooltip }}">
                                                        @endif
                                                        @if ($ride->luggage == ($findRidePage->luggage_option1->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->luggage_option1_tooltip }}">
                                                        @elseif ($ride->luggage == ($findRidePage->luggage_option2->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->luggage_option2_tooltip }}">
                                                        @elseif ($ride->luggage == ($findRidePage->luggage_option3->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->luggage_option3_tooltip }}">
                                                        @elseif ($ride->luggage == ($findRidePage->luggage_option4->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->luggage_option4_tooltip }}">
                                                        @elseif ($ride->luggage == ($findRidePage->luggage_option5->features_setting_id ?? null))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->luggage_option5_tooltip }}">
                                                        @endif
                        ...
                        @if ($paginatedRides->filter(fn($ride) => $ride->type === 'otherRide')->count() > 0)
                            <div class="border-b border-gray-400 flex flex-col items-center justify-center pt-6">
                                @if ($paginatedRides->filter(fn($ride) => $ride->type === 'ride')->count() <= 0)
                                    <h3 class="text-primary">
                                        {{ $findRidePage->no_rides_found_extra_ride_label ?? 'Sorry, we couldn\'t find any Folk Rides matching your search.' }}
                                    </h3>
                                @endif
                                <h3 class="text-primary">
                                    {{ $findRidePage->more_rides_pink_ride_label ?? 'More rides from' }}
                                    {{ $request->from }} {{ $findRidePage->to_extra_care_ride_label ?? 'to' }}
                                    {{ $request->to }}.
                                    {{ $findRidePage->imp_extra_care_ride_label ?? 'Important: these are NOT Folk Ride' }}
                                </h3>
                            </div>
                                                ...
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    @if (in_array($findRidePage->ride_features_option1->features_setting_id ?? null, explode('=', $ride->features)))
                                                        <span class="ml-2 inline-block cursor-help"
                                                            ...
                                                            title="{{ $postRidePage->features_option1_tooltip ?? '' }}">
                                                            <img class="w-8 h-8"
                                                        ...
                                                        <!-- <div class="sups inline-flex relative">
                                                                        ...
                                                                        >
                                                                            <p class="text-white font-semibold text-start text-sm lg:text-base">
                                                                                {!! nl2br($findRidePage->firm_cancellation_tooltip) ??
                                                                                    'This ride has the Firm cancellation policy, so its booking price is reduced by 10%' !!}
                                                                            </p>
                                                                        </div>
                                                                    </div> -->
                                                    ...
                                                    <div class="flex flex-nowrap items-center gap-1 md:gap-2">
                                                        ...
                                                        @if (in_array($findRidePage->ride_features_option3->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option3_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option8->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option8_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option9->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option9_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option10->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option10_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option11->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option11_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option12->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option12_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option13->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option13_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option14->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option14_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option15->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option15_tooltip }}">
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option16->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option16_tooltip }}">
                                                        @endif
                                                        @if (in_array($postRidePage->features_option4->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option4_tooltip }}">
                                                        @endif
                                                        @if (in_array($postRidePage->features_option5->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option5_tooltip }}">
                                                        @endif
                                                        @if (in_array($postRidePage->features_option6->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option6_tooltip }}">
                                                        @endif
                                                        @if (in_array($postRidePage->features_option7->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help"
                                                                ...
                                                                title="{{ $postRidePage->features_option7_tooltip }}">
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                ...
                <div class="hidden overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center"
                    ...
                    <div class="fixed inset-0 z-10 w-screen overflow-y-auto flex items-center justify-center">
                        <div
                            ...
                            <div class="px-4 pb-6 pt-4 sm:flex sm:flex-row-reverse sm:px-6 justify-center">
                                <button type="button" onclick="toggleModal1('modal-id2')"
                                    class="bg-greenXS hover:bg-greenXS text-white text-base md:text-lg rounded font-FuturaMdCnBT px-5 py-2 border border-greenXS hover:border-greenXS">
                                    Close
                                </button>
                            </div>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        const folkRideModalMsgFemale = {!! json_encode($successMessage->female_user_message ?? 'Only females can select this ride') !!};
        const folkRideModalMsgCompleteAddress = {!! json_encode(
            $successMessage->complete_address_passenger_message ?? 'Only passengers with complete address can select this ride',
        ) !!};
        const folkRideModalMsgStar5 = {!! json_encode($successMessage->star5_passenger_message ?? 'Only 5 star passengers can select this ride') !!};
        const folkRideModalMsgStar5CompleteAddress = {!! json_encode(
        ...
        const folkRideModalMsgPassengerWithReviews = {!! json_encode($successMessage->passenger_with_review_message ?? 'Driver only want passengers with reviews') !!};
        const folkRideModalMsgStar3 = {!! json_encode(
            $successMessage->star3_passenger_message ?? 'Driver want only passengers with-3 star reviews above',
        ) !!};
        const folkRideModalMsgStar4 = {!! json_encode(
            $successMessage->star4_passenger_message ?? 'Driver want only passengers with-4 star reviews above',
        ) !!};
        const folkRideModalMsgStar5Driver = {!! json_encode(
        ...
        window.initFolkRidePlaces = function() {
            ...
            fromAutocompleteFolkRide = new google.maps.places.Autocomplete(fromInput, {
            ...
            toAutocompleteFolkRide = new google.maps.places.Autocomplete(toInput, {
                componentRestrictions: {
                    country: 'ca'
                },
                types: ['(cities)'],
            ...
            fromAutocompleteFolkRide.addListener('place_changed', function() {
                ...
                if (place.address_components && place.place_id) {
                    ...
                    isSelectingFromDropdownFolkRide = true;
                    ...
                    setTimeout(function() {
                        isSettingPlaceValueFolkRide = false;
                        isSelectingFromDropdownFolkRide = false;
                    }, 100);
                }
            });
            ...
            fromInput.addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    ...
                    resolveTypedCityValueFolkRide(this.value, 'from').then(function() {
            ...
            toInput.addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    event.preventDefault();
                    resolveTypedCityValueFolkRide(this.value, 'to').then(function() {
                        navigateToSearchRoute1();
                    });
                }
            });
                        ...
                        if (currentValue !== '' && fromInputError) {
                            ...
                            if (tooltipError) {
                                tooltipError.textContent = currentValue === '' ?
                                    errorFromRequiredFolkRide : errorCityMissingFolkRide;
                            }
                ...
                setTimeout(async function() {
                    ...
                    if (currentValue === '' || !selectedToPlace || currentValue !== selectedToPlace
                        ...
                        if (currentValue !== '' && toInputError) {
                            ...
                            if (tooltipError) {
                                tooltipError.textContent = currentValue === '' ?
                                    errorToRequiredFolkRide : errorCityMissingFolkRide;
                            }
        ...
        function SearchRoute(SearchfromValue, SearchtoValue) {
            ...
            let searchUrl =
                `{{ route('folk_ride', ['lang' => $selectedLanguage->abbreviation]) }}?from=${SearchfromValue}&to=${SearchtoValue}&date=&driver_age=&driver_rating=&driver_phone=&driver_name=&passenger_rating=&payment_method=&vehicle_type=&features=${featuresParam}&luggage=&smoking=&pets=&hide_full_rides=${hideFullRides}`;
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ env('GOOGLE_API_KEY') }}&libraries=places&callback=initFolkRidePlaces"
        async defer></script>
@endsection
