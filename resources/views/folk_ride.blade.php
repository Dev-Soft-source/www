...
@section('style')
    ...
    <style>
        ...
        .folk-ride-faq__item:last-child { margin-bottom: 0; }
                    ...
                    @elseif ($paginatedRides && $paginatedRides->count() > 0)
                        @if ($paginatedRides->filter(fn($ride) => $ride->type === 'ride')->count() > 0)
                            <div class="flex flex-col items-center justify-center border-b border-gray-400">
                                <h3 class="text-primary">{{ $findRidePage->search_results_extra_care_ride_label ?? 'Search results for Folk Ride' }}</h3>
                            </div>
                                    ...
                                    <a
                                        ...
                                        @elseif (auth()->user() &&
                                            ...
                                            @if ($filteredRatings->count() === 0)
                                                href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgPassengerWithReviews)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}"
                                            @endif
                                        @elseif (auth()->user() &&
                                            ...
                                            @if ($totalAverage < 3)
                                                href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar3)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}"
                                            @endif
                                        @elseif (auth()->user() &&
                                            ...
                                            @if ($totalAverage < 4)
                                                href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar4)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}"
                                            @endif
                                        @elseif (auth()->user() &&
                                            ...
                                            @if ($totalAverage < 4.5)
                                                href="javascript:void(0);" onclick="toggleModal1('modal-id1', folkRideModalMsgStar5Driver)"
                                            @else
                                                href="{{ route('ride_detail', ['lang' => $selectedLanguage->abbreviation, 'departure' => $ride->detail->departure, 'destination' => $ride->detail->destination, 'id' => $ride->id]) }}"
                                            @endif
                                        @else
                                            ...
                                            <div class="border-t border-gray-300 grid grid-cols-4 divide-x divide-gray-300">
                                                ...
                                                <div class="col-span-4 p-4 flex justify-start items-center no-scrollbar overflow-x-auto overflow-y-hidden space-x-2 md:space-x-4">
                                                    ...
                                                    <div class="flex flex-nowrap items-center gap-1 md:gap-2">
                                                        ...
                                                        @endif
                                                        @if (in_array($findRidePage->ride_features_option2->features_setting_id ?? null, explode('=', $ride->features)))
                                                            <img class="w-8 h-8 cursor-help" src="{{asset('home_page_icons/' . $findRidePage->ride_features_option2->icon)}}" alt=""
                                                                data-tippy-content="{{ $postRidePage->features_option2_tooltip }}" title="{{ $postRidePage->features_option2_tooltip }}">
                                                        @endif
...
@section('script')
    ...
    <script>
        ...
        const folkRideModalMsgCompleteAddress = {!! json_encode($successMessage->complete_address_passenger_message ?? 'Only passengers with complete address can select this ride') !!};
        ...
        const folkRideModalMsgStar5CompleteAddress = {!! json_encode($successMessage->star5_passenger_with_complete_address_message ?? 'Only 5 star passengers with complete address can select this ride') !!};
        ...
        const folkRideModalMsgStar3 = {!! json_encode($successMessage->star3_passenger_message ?? 'Driver want only passengers with-3 star reviews above') !!};
        const folkRideModalMsgStar4 = {!! json_encode($successMessage->star4_passenger_message ?? 'Driver want only passengers with-4 star reviews above') !!};
        const folkRideModalMsgStar5Driver = {!! json_encode($successMessage->star5_passenger_message ?? 'Driver want only passengers with-5 star reviews above') !!};
        ...
        window.initFolkRidePlaces = function() {
            ...
            fromInput.addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    ...
                    resolveTypedCityValueFolkRide(this.value, 'from').then(function() { navigateToSearchRoute1(); });
                }
            });
            toInput.addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    event.preventDefault();
                    resolveTypedCityValueFolkRide(this.value, 'to').then(function() { navigateToSearchRoute1(); });
                }
            });
