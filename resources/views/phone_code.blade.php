...
@section('content')
    <div class="grid grid-cols-12 gap-4 container mx-auto my-14">
        ...
        <div class="bg-white border rounded p-4 border-gray-200 w-full col-span-12 md:col-span-9 shadow">
            @if (session('message'))
                <div id="deleteModal" class="relative z-50 delete-modal" aria-labelledby="modal-title" role="dialog"
                    aria-modal="true">
                    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity delete-modal-backdrop"
                        id="deleteModalBackdrop" onclick="closeDeleteModal()"></div>
                        ...
                        <div
                            ...
                            <div
                                ...
                                <div class="px-4 pb-6 pt-4 flex items-center space-x-2 sm:space-x-4 sm:px-6 justify-center">
                                    ...
                                    <a href="#" onclick="closeDeleteModal()"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 w-full md:w-1/2 gap-4 mt-4">
                @foreach ($phone_numbers as $phone_number)
                    <div>
                        ...
                        @endif
                        <div
                            class="flex justify-between items-center mt-1 border p-1.5 w-full text-base lg:text-lg text-left bg-gray-50 rounded border-gray-300 focus:ring-none focus:outline-none focus:border-blue-600">
                            ...
                            @if ($phone_number->verified === '1')
                                <div>
                                    ...
                                    <a href="{{ route('phone.destroy', $phone_number->id) }}"
                                        class="button-exp-fill py-1.5">Delete</a>
                                </div>
                            @else
                                <div>
                                    <a href="{{ route('send_verification_code', $phone_number->id) }}"
                                        class="button-exp-fill py-1.5">Send verification code</a>
                                    <a href="{{ route('phone.destroy', $phone_number->id) }}"
                                        class="button-exp-fill py-1.5">Delete</a>
                                </div>
                            @endif
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('click', function(event) {
                    ...
                    if (deleteModal && !deleteModal.classList.contains('delete-modal')) {
                        ...
                        if (modalContent && (!modalContent.contains(event.target) || (deleteModalBackdrop &&
                                deleteModalBackdrop.contains(event.target)))) {
                            closeDeleteModal();
                        }
                    }
                    if (verifyModal && !verifyModal.classList.contains('hidden')) {
                        const modalContent = verifyModal.querySelector('.animate__fadeIn');
                        if (modalContent && (!modalContent.contains(event.target) || (verifyModalBackdrop &&
                                verifyModalBackdrop.contains(event.target)))) {
                            closeModal();
                        }
                    }
                });
