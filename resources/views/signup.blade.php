...
@section('content')
            ...
            <div class="bg-white border border-gray-100 p-4 shadow rounded-md sm:px-10">
                ...
                <div class="sm:mx-auto sm:w-full sm:max-w-xl md:min-w-[26rem]">
                    ...
                    <form id="signupForm" method="POST"
                        action="{{ route('signup.store', ['lang' => $selectedLanguage->abbreviation]) }}">
                        @csrf

                        <input type="hidden" value="{{ isset($uuid) && !is_null($uuid) ? $uuid : 0 }}" name="uuid"
                            id="uuid">
                        <div>
                            <div class="">
                            ...
                            <label for="password" class="flex items-center space-x-1 font-FuturaMdCnBT">
                                ...
                                <div class="relative sups inline-flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        ...
                                        class="w-5 h-5 text-blue-500 cursor-help hover:text-blue-700">
                ...
                <div class="flex items-center justify-center space-x-1.5 md:space-x-4 my-4">
                    ...
                    <a href="{{ route('signup.redirectToProvider', ['lang' => $selectedLanguage->abbreviation, 'provider' => 'linkedin', 'intent' => 'signup']) }}"
                        class="w-12 md:w-14 h-12 md:h-14 rounded border flex justify-center items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" x="0px" y="0px" width="100"
    ...
    <div id="my-modal" class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true"
        style="display: {{ session('showModal') ? 'block' : 'none' }};">
                    ...
                    if (data.emailSent === false && resendSection) {
                        ...
                        if (emailErrorMessage) {
                            emailErrorMessage.textContent =
                                'We encountered an issue sending the verification email. Please use the button below to request a new one.';
                        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setTimeout(function() {
                document.querySelectorAll('.tooltip:not(.hidden)').forEach(function(tooltip) {
                    ...
                    var hasText = tooltip.querySelector('p') && tooltip.querySelector('p')
                        .textContent.trim() !== '';
