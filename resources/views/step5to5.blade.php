...
@section('content')
    <div class="mx-auto max-w-2xl lg:max-w-4xl my-6">
                        ...
                        <div
                            ...
                            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                <div class="text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    <h2 class="font-FuturaMdCnBT text-gray-700 mb-4" id="modal-title">
                                        {!! session('heading') !!}</h2>
        ...
        <div class="bg-white rounded p-4 w-full col-span-12 md:col-span-9 mx-auto">
            <div class="bg-white border border-gray-100 pb-8 px-4 shadow rounded-md sm:px-10 my-4">
                <div class="pb-2 flex items-center justify-center">
                    <h1 class="font-FuturaMdCnBT mt-10 text-primary text-3xl md:text-4xl lg:text-5xl mb-4">
                        @isset($step5Page->main_heading)
                            {{ $step5Page->main_heading }}
                        @endisset
                    </h1>
                </div>
                <p class="text-black">
                    @isset($step5Page->main_label)
                        {!! $step5Page->main_label !!}
                    @endisset
                </p>
                                ...
                                <div class="w-full lg:w-3/6">
                                    <label class="text-gray-700 font-FuturaMdCnBT mb-2 block">
                                        @isset($step5Page->select_country_label)
                                            {{ $step5Page->select_country_label }}
                                        @endisset
                                    </label>
                                    <select name="country" id="country-dropdown"
                                        class="font-FuturaMdCnBT bg-white text-base block mt-1 border p-1.5 w-full rounded border-gray-300 focus:ring-none focus:outline-none focus:border-blue-600 placeholder:text-gray-900 {{ $errors->has('country') ? 'border-red-500' : '' }}">
        ...
        <div id="skipModal" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity z-50"
            ...
            <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" id="skipModalBackdrop"
                    onclick="closeSkipConfirmation()"></div>
                <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0 w-full">
                    <div
                        ...
                        <button type="button" onclick="closeSkipConfirmation()"
                            class="absolute top-3 right-3 text-gray-400 hover:text-gray-500">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div id="saveUnverifiedModal" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity z-50 flex"
            aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" id="saveUnverifiedModalBackdrop"
                    onclick="closeSaveUnverifiedModal()"></div>
                <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0 w-full">
                    <div
                        class="relative animate__animated animate__fadeIn transform overflow-hidden rounded-2xl bg-white text-center shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg w-full modal-border">
                        <button type="button" onclick="closeSaveUnverifiedModal()"
                            class="absolute top-3 right-3 text-gray-400 hover:text-gray-500">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12"></path>
        ...
        <div id="whatsappUnavailableModal" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity z-50"
            ...
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
                onclick="closeWhatsAppUnavailableModal()"></div>
                    ...
                    <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0 w-full">
                        <div
                            ...
                            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                <div class="flex flex-col items-center justify-center mt-2">
                                    <h3 class="font-FuturaMdCnBT text-gray-700 mb-4 text-center">
                                        @isset($step5Page->verify_code_heading)
                                            {{ $step5Page->verify_code_heading }}
                                        @endisset
                                    </h3>
                                    <p class="text-center mt-2">
                                        @isset($step5Page->verify_code_label_1)
                                            {{ $step5Page->verify_code_label_1 }}
                                        @endisset
                                    </p>
                                    <div class="flex justify-center mt-4 space-x-2">
                                        <input type="text" name="code[]" maxlength="1"
                                            class="font-FuturaMdCnBT w-10 h-10 text-center block mt-1 border p-1.5 text-base rounded border-gray-300 focus:ring-none focus:outline-none focus:border-blue-600">
                                        <input type="text" name="code[]" maxlength="1"
                                            class="font-FuturaMdCnBT w-10 h-10 text-center block mt-1 border p-1.5 text-base rounded border-gray-300 focus:ring-none focus:outline-none focus:border-blue-600">
                                        <input type="text" name="code[]" maxlength="1"
                                            class="font-FuturaMdCnBT w-10 h-10 text-center block mt-1 border p-1.5 text-base rounded border-gray-300 focus:ring-none focus:outline-none focus:border-blue-600">
                            ...
                            <div class="px-4 pb-6 pt-4 flex flex-wrap items-center justify-center">
                                <button type="submit"
                                    class="inline-flex w-42 justify-center rounded bg-primary px-3 py-2 font-FuturaMdCnBT text-lg text-white hover:text-white hover:shadow-lg shadow-sm hover:bg-blue-400">
                                    @isset($step5Page->verify_button_label_1)
                                        {{ $step5Page->verify_button_label_1 }}
                                    @endisset
                                </button>
                            </div>
                        </div>
