                                        ...
                                        <select id="vehicle_id_select" name="vehicle_id"
                                            ...
                                            @foreach ($vehicles as $vehicle)
                                                <option value="{{ $vehicle->id }}"
                                                    ...
                                                    {{ $vehicle->year }}@if ($vehicle->type_label)
                                                        / {{ $vehicle->type_label }}
                                                    @endif
                                                </option>
                                            @endforeach
...
@section('script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function syncRideDisclaimers() {
                const pinkChecked = !!(pinkRideCheckbox && pinkRideCheckbox.checked);
                const extraChecked = !!(extraCareRideCheckbox && extraCareRideCheckbox.checked);

                if (pinkRideDisclaimer) {
                    pinkRideDisclaimer.classList.toggle('hidden', !pinkChecked);
                }

                if (extraCareRideDisclaimer) {
                    extraCareRideDisclaimer.classList.toggle('hidden', !extraChecked);
                }

                if (extraCareDisclaimerNumber) {
                    extraCareDisclaimerNumber.textContent = pinkChecked ? '6.' : '5.';
                }
            }

            if (pinkRideCheckbox) {
                pinkRideCheckbox.addEventListener('change', syncRideDisclaimers);
            }

            if (extraCareRideCheckbox) {
                extraCareRideCheckbox.addEventListener('change', syncRideDisclaimers);
            }
