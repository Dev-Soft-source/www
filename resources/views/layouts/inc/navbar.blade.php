        ...
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0 w-full">
                <div class="relative animate__animated animate__fadeIn transform overflow-hidden rounded-2xl bg-white text-center shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg modal-border1">
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <h3 class="font-FuturaMdCnBT text-gray-700 mb-4" id="phone-required-modal-title">{{ optional($postRidePage)->phone_required_modal_heading ?? 'Action Required' }}</h3>
                        <p class="text-gray-800 text-base text-left">{{ optional($postRidePage)->phone_required_modal_body_before ?? 'You must verify your phone number before posting any ride. Please do this in ' }} <a href="{{ route('phone', ['lang' => optional($selectedLanguage)->abbreviation ?? 'en']) }}" class="underline font-medium hover:no-underline text-blue-600">{{ optional($postRidePage)->phone_required_modal_link_text ?? 'Dashboard → My Phone Number' }}</a>{{ optional($postRidePage)->phone_required_modal_body_after ?? '.' }}</p>
                        <div class="mt-6 flex flex-wrap gap-2 justify-center">
                            <button type="button" onclick="closePhoneRequiredModal()" class="button-exp-fill font-FuturaMdCnBT">{{ optional($postRidePage)->phone_required_modal_close_btn ?? $successMessage->cancel_button ?? 'Close' }}</button>
                            <a href="{{ route('phone', ['lang' => optional($selectedLanguage)->abbreviation ?? 'en']) }}" class="button-exp-fill bg-primary hover:bg-primary font-FuturaMdCnBT no-underline">{{ optional($postRidePage)->phone_required_modal_phone_btn ?? 'My Phone Number' }}</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-x-5 justify-between items-center">
                ...
                <div>
                    @php
                        ...
                        $navbarMenu = getNavbarMenuSetting($selectedLanguage ?? 1);
                        $profileDropdownItems = $navbarMenu->profileDropdownItems ?? [];
                        $guestNavItems = $navbarMenu->guestNavItems ?? [];
                        $studentPage = App\Models\StudentPageSettingDetail::where(
                            'language_id',
                            $selectedLanguage->id,
                        )->first();
                        $postRidePage = App\Models\PostRidePageSettingDetail::where(
                            'language_id',
                            $selectedLanguage->id,
                        )->first();
                        $findRidePage = App\Models\FindRidePageSettingDetail::where(
                            'language_id',
                            $selectedLanguage->id,
                        )->first();
            ...
            <div class="flex items-center justify-end space-x-2 w-1/2">
                @if (auth()->check())
                    ...
                    <div class="relative">
                            ...
                            <button id="dropdownNotificationButton" data-dropdown-toggle="dropdown_notification"
                                ...
                                <div class="relative">
                                    ...
                                    @if ($notifications && $notifications->where('is_read', 0)->count() > 0)
                                        <div
                                            class="absolute -top-3 -right-2 p-1 bg-red-500 rounded-full text-white text-[7pt] z-10 h-5 w-5 flex items-center justify-center">
                        ...
                        <!-- Dropdown menu -->
                        <div id="dropdown_notification"
                            class="animate__animated animate__fadeIn absolute right-4 z-30 hidden bg-white divide-y divide-gray-100 rounded shadow w-80 top-10">
                            <div class="p-4">
                    ...
                    <div class="relative">
                        <div class="border-l pl-3 flex items-center gap-1">
                            ...
                            <div class="flex items-center justify-center">
                                    <img class="h-10 w-10 border-2 border-blue-300 rounded-full object-cover"
                                        src="{{ auth()->user()->profile_image ?? asset('assets/male.png') }}" alt="">
                                </div>
                            </button>
                        </div>
                        <!-- Dropdown menu -->
                        <div id="dropdown_profile"
                            class="animate__animated animate__fadeIn absolute right-4 z-30 hidden bg-white divide-y divide-gray-100 rounded shadow w-40">
                            <ul class="py-2 text-sm text-gray-700 font-FuturaMdCnBT"
                                ...
                                @foreach ($profileDropdownItems as $item)
                                    ...
                                    <li>
                                        @if ($link === 'logout')
                                            <a href="#" type="button" onclick="toggleModal('logout-modal')"
                                                class="flex gap-2 items-center px-4 py-2 hover:bg-gray-100 text-black">
                                        ...
                                        @elseif ($link && \Illuminate\Support\Facades\Route::has($link))
                                            ...
                                            <a href="{{ route($link, ['lang' => $langAbbr]) }}"
                                                ...
                                                @elseif ($link === 'my_rides')
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                        height="16" fill="currentColor"
                                                        class="bi bi-car-front w-6 h-6 p-0.5" viewBox="0 0 16 16">
                                                        <path
                                                ...
                                                @elseif ($link === 'my_chats')
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                        fill="currentColor" class="w-6 h-6 p-0.5 text-primary">
                ...
                @else
                    @foreach ($guestNavItems as $item)
                        ...
                        @if ($link && \Illuminate\Support\Facades\Route::has($link))
                            <div class="relative">
                                <a href="{{ route($link, ['lang' => $langAbbr]) }}"
                                    class="px-2 py-1.5 button-exp-no-fill ml-1 flex gap-2 items-center !text-base xl:!text-lg">
                                    @if ($link === 'login')
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <nav class="bg-gray-50 border-gray-200 md:hidden">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4">
            ...
            <div class="flex gap-2 items-center">
                @if (auth()->check())
                    <div class="">
                        <button data-collapse-toggle="profilebar-default" type="button"
                            class="pr-2 border-r flex items-center gap-1" aria-controls="profilebar-default"
                            aria-expanded="false">
                            <div class="h-10 w-10 bg-gray-500 rounded-full flex items-center justify-center"><img
                                    class="rounded-full h-full" src="{{ auth()->user()->profile_image }}"
                                    alt=""></div>
                        </button>
                    </div>
                @endif
                ...
                <div class="relative">
                    <button id="dropdownMobileButton" data-dropdown-toggle="dropdown_mobile"
                        class="px-2 py-1.5 border border-gray-300 rounded ml-2 flex gap-2 items-center"
                        type="button"><img class="h-4"
                            src="{{ $selectedLanguage->flag_icon ?? 'assets/flag.png' }}"
                            alt=""><span>{{ $selectedLanguage->name ?? 'Eng' }}</span></button>
                    <!-- Dropdown menu -->
                    <div id="dropdown_mobile"
                        class="absolute right-0 z-30 hidden bg-white divide-y divide-gray-100 rounded shadow w-32">
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-default">
            <ul
                class="font-FuturaMdCnBT text-blue-600 flex flex-col space-y-2 mt-2 pb-4 border-t pt-2 md:flex-row md:space-x-8 md:mt-0 md:border-0">
                @foreach ($topMenuItems as $item)
                    @php
                        $link = $item['link'] ?? '';
                        $name = $item['name'] ?? '';
                            ...
                            <a href="{{ route($link, ['lang' => $langAbbr]) }}"
                                ...
                                @elseif ($link === 'my_rides')
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                        fill="currentColor" class="bi bi-car-front w-6 h-6 p-0.5"
                                        viewBox="0 0 16 16">
                                        <path
                                            d="M4 9a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm10 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0ZM6 8a1 1 0 0 0 0 2h4a1 1 0 1 0 0-2H6ZM4.862 4.276 3.906 6.19a.51.51 0 0 0 .497.731c.91-.073 2.35-.17 3.597-.17 1.247 0 2.688.097 3.597.17a.51.51 0 0 0 .497-.731l-.956-1.913A.5.5 0 0 0 10.691 4H5.309a.5.5 0 0 0-.447.276Z" />
                                        <path
                                ...
                                @elseif ($link === 'my_chats')
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                        class="w-6 h-6 p-0.5 text-primary">
                                        <path fill-rule="evenodd"
                                            d="M12 2.25c-2.429 0-4.817.178-7.152.521C2.87 3.061 1.5 4.795 1.5 6.741v6.018c0 1.946 1.37 3.68 3.348 3.97.877.129 1.761.234 2.652.316V21a.75.75 0 0 0 1.28.53l4.184-4.183a.39.39 0 0 1 .266-.112c2.006-.05 3.982-.22 5.922-.506 1.978-.29 3.348-2.023 3.348-3.97V6.741c0-1.947-1.37-3.68-3.348-3.97A49.145 49.145 0 0 0 12 2.25ZM8.25 8.625a1.125 1.125 0 1 0 0 2.25 1.125 1.125 0 0 0 0-2.25Zm2.625 1.125a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0Zm4.875-1.125a1.125 1.125 0 1 0 0 2.25 1.125 1.125 0 0 0 0-2.25Z"
                                            clip-rule="evenodd" />
                                    </svg>
