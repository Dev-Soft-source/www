            ...
            <div class="flex items-center justify-end space-x-2 w-1/2">
                ...
                <div class="relative">
                    <a href="{{ route('coffee_on_wall', ['lang' => optional($selectedLanguage)->abbreviation]) }}" class="px-2 py-1.5 button-exp-no-fill ml-2 flex gap-2 items-center !text-base xl:!text-lg">
                        <span>{{ getTranslatedText('nav_coffee_on_wall', $selectedLanguage ?? 1, [], 'Coffee on the Wall') }}</span>
                    </a>
                </div>
                <div class="relative">
                    ...
                    <div id="dropdown_notification" class="animate__animated animate__fadeIn absolute right-4 z-30 hidden bg-white divide-y divide-gray-100 rounded shadow w-80 top-10">
                        <div class="p-4">
                            <a href="{{ route('notifications', ['lang' => $selectedLanguage->abbreviation]) }}" class="w-full text-center font-FuturaMdCnBT bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">
                                {{ getTranslatedText('nav_view_all', $selectedLanguage ?? 1, [], 'View All') }}
                            </a>
                        </div>
                    ...
                    <div id="dropdown_profile" class="animate__animated animate__fadeIn absolute right-4 z-30 hidden bg-white divide-y divide-gray-100 rounded shadow w-40">
                        <ul class="py-2 text-sm text-gray-700 font-FuturaMdCnBT" aria-labelledby="dropdownProfileButton">
                            @foreach ($profileDropdownItems as $item)
                                @php $link = $item['link'] ?? ''; $name = $item['name'] ?? ''; @endphp
                                <li>
                                    @if ($link === 'logout')
                                    ...
                                    @elseif ($link && \Illuminate\Support\Facades\Route::has($link))
                                        <a href="{{ route($link, ['lang' => $langAbbr]) }}" class="flex gap-2 items-center px-4 py-2 hover:bg-gray-100">
                                            @if ($link === 'profile')
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                ...
                @foreach ($guestNavItems as $item)
                    ...
                    @if ($link && \Illuminate\Support\Facades\Route::has($link))
                        <div class="relative">
                            <a href="{{ route($link, ['lang' => $langAbbr]) }}" class="px-2 py-1.5 button-exp-no-fill ml-1 flex gap-2 items-center !text-base xl:!text-lg">
                                @if ($link === 'login')
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    ...
                    <div class="flex items-center gap-2 justify-center mt-2">
                        @foreach ($guestNavItems as $item)
                            ...
                            @if ($link === 'login' && !Auth::check() && \Illuminate\Support\Facades\Route::has($link))
                                <a href="{{ route($link, ['lang' => $langAbbr]) }}" class="button-exp-no-fill flex items-center justify-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                                    </svg>
                                    <span>{{ $name }}</span>
