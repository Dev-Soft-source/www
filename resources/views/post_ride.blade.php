                    ...
                    <div
                        ...
                        <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            ...
                            <div class="mt-6 flex flex-wrap gap-2 justify-center">
                                <button type="button" onclick="closePinkExtraAddressRequiredModal()"
                                    class="button-exp-fill font-FuturaMdCnBT">{{ $siteText['close_btn_text'] }}</button>
                    ...
                    <div class="bg-white p-4 space-y-3">
                                            ...
                                            <div class="text-center sm:mt-0 sm:text-left">
                                                <div class="mt-2 w-full">
                                                    <p id="delete-stop-modal-title-post"
                                                        class="can-exp-p text-center text-xl">
                                                        {{ $postRidePage->delete_stop_text ?? 'Delete Stop?' }}</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div
                                            ...
                                            <button type="button" id="delete-stop-no-post"
                                                class="w-24 bg-blue-600 p-2 rounded-md text-white hover:bg-blue-700">{{ $siteText['cancel_btn_text'] }}</button>
                                            <button type="button" id="delete-stop-yes-post"
                                                class="w-24 bg-red-600 p-2 rounded-md text-white hover:bg-red-700">{{ $siteText['remove_btn_text'] }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-white rounded-lg overflow-hidden shadow-3xl mt-4" id="stops-section-wrapper"
                            ...
                            <button type="button" id="add-more-spots-toggle"
                                ...
                                <h3 class="text-2xl">
                                    @if (isset($postRidePage->stop_along_the_way_label))
                                        {{ $postRidePage->stop_along_the_way_label }}
                                    @elseif(isset($postRidePage->add_more_from_to))
                                        {{ $postRidePage->add_more_from_to }}
                                    @else
                                        Stops Along the Way (Optional) @endif
                                </h3>
                            ...
                            <div id="add-more-spots-panel" class="add-more-spots-panel" role="region"
                                ...
                                <div class="add-more-spots-panel-inner bg-white p-4">
                                    ...
                                    <h4 class="text-xl font-medium text-gray-900 mt-4 mb-3">
                                        {{ $postRidePage->stop_along_the_way_label ?? 'Stops Along the Way' }}: <span
                                            ...
                                            @foreach ($stopsForDisplayPost as $idx => $stopValue)
                                                ...
                                                <div class="flex items-center gap-3 stop-row"
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        onclick="confirmDeleteStopPostRide(this)"
                                                        title="{{ $postRidePage->delete_stop_text ?? 'Delete stop' }}"
                                                        aria-label="{{ $postRidePage->delete_stop_text ?? 'Delete stop' }}">
                                    ...
                                    <button type="button" onclick="addStopRowPostRide();"
                                        class="button-exp-fill flex-shrink-0 whitespace-nowrap mb-4">{{ $postRidePage->add_stop_btn_label ?? '+ Add Stop' }}</button>
                                    <div class="flex items-center gap-2 mb-3">
                                        <h4 class="text-gray-900 text-xl font-medium">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            try {
                ...
                if (typeof hideFromTooltipErrorsPostRide === 'function')
    ...
    function hideFromTooltipErrorsPostRide() {
        var a = document.getElementById('fromInputError');
        if (a) a.classList.add('hidden');
        var b = document.getElementById('from-error-laravel');
        if (b) b.classList.add('hidden');
    }

    function hideToTooltipErrorsPostRide() {
        var a = document.getElementById('toInputError');
        if (a) a.classList.add('hidden');
        var b = document.getElementById('to-error-laravel');
        if (b) b.classList.add('hidden');
    }
    ...
    function resolveTypedCityValuePostRide(rawValue, target) {
        ...
        return new Promise(function(resolve) {
            geocoderPostRide.geocode({
                ...
                if (target === 'from') hideFromTooltipErrorsPostRide();
    ...
    function addStopRowPostRide() {
        ...
        row.innerHTML = '<div class="flex flex-row gap-2 items-stretch flex-1 min-w-0">' +
            ...
            @json($postRidePage->stop_placeholder ?? '') + '">' +
            ...
            nextIndex + '" rows="1" placeholder="' + @json($postRidePage->pickup_off_placeholder ?? 'pick up / drop off') +
            ...
            '</div>' +
            '<button type="button" class="stop-delete-btn flex-shrink-0 p-2 text-red-600 hover:text-red-700 hover:bg-red-50 rounded focus:outline-none focus:ring-2 focus:ring-red-400" onclick="confirmDeleteStopPostRide(this)" title="' +
            @json($postRidePage->delete_stop_text ?? 'Delete stop') + '" aria-label="' + @json($postRidePage->delete_stop_text ?? 'Delete stop') + '">' +
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (fromInput0) {
            ...
            fromInput0.addEventListener('input', function() {
                if (typeof updateStopsOriginDestinationLabelsPostRide === 'function')
                    updateStopsOriginDestinationLabelsPostRide();
                if (this.value.trim() !== '' && typeof hideFromTooltipErrorsPostRide === 'function')
                    hideFromTooltipErrorsPostRide();
            });
        ...
        if (toInput0) {
            ...
            toInput0.addEventListener('input', function() {
                if (typeof updateStopsOriginDestinationLabelsPostRide === 'function')
                    updateStopsOriginDestinationLabelsPostRide();
                if (this.value.trim() !== '' && typeof hideToTooltipErrorsPostRide === 'function')
                    hideToTooltipErrorsPostRide();
            });
