@php
    $iconClass = $iconClass ?? 'w-12 h-12';
    $profile_image = $user->profile_image;
    if ($user->isVerifiedStudent()) {
        $kind = 'student';
    } elseif ($user->isPendingStudent()) {
        $kind = 'pending';
    } else {
        $kind = 'default';
    }
...
<div class="relative {{ $iconClass }}">
    ...
    @if ($kind != 'default')
        <div
            class="absolute -top-1 -right-1 w-5 h-5 bg-{{ $kind == 'student' ? 'blue' : 'gray' }}-500 rounded-full border-2 border-white flex items-center justify-center">
            {{-- is Student --}}

                <svg class="w-3 h-3 text-white" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg"
