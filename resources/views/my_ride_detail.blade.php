    ...
    <div class="container mx-auto my-10 xl:my-14 px-4 xl:px-0">
        @php
            ...
            if ($ride->isCancelled()) {
                $note = [
                    'bg' => 'red',
                    'text' => $rideDetailPage->ride_canceller_by_driver ?? 'This ride was cancelled by the driver',
                ];
        ...
        <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-y-4 md:gap-4">
                                    ...
                                    @foreach ($ride->bookings()->requested()->get() as $booking)
                                        @if ($booking->passenger)
                                            <div
                                                ...
                                                <div
                                                    class="border-b border-slate-300 px-4 py-2 font-FuturaMdCnBT text-2xl hidden">
                                                    {{ $rideDetailPage->booking_request_heading ?? 'Booking request' }}
                                                </div>
                                                <div class="p-4">
                                                    <div class="flex items-start">
                                                        <div class="mr-2 2xl:mr-3 mt-2">
                                                            @include('partials.passenger_avatar', [
                                                                'user' => $booking->passenger,
                                                                'iconClass' => 'w-12 2xl:w-16 h-12 2xl:h-16',
                                                            ])
                                                        </div>
                                                        <div>
                                                            ...
                                                            @if ($isMultiStopsRide && $booking->from_stop_id)
                                                                <span
                                                                    class="text-gray-700 text-sm">{{ $booking->fromStop->label }}
                                                                    → {{ $booking->toStop->label }}</span>
                    ...
                    <div
                        ...
                        <div class="p-4 items-baseline">
                            <div class="flex flex-wrap items-end gap-3">
                                @php
                                    $payment_method = $ride->resolvePaymentMethodOption(
                                        $rideFeatureOptions['payment_method'] ?? [],
                                    );
                                @endphp
                                <h4 class="font-medium text-xl xl:text-2xl text-left text-black font-FuturaMdCnBT">
                        ...
                        <div class="p-4 items-baseline">
                            <div class="flex flex-wrap items-end gap-3">
                                @php
                                    $booking_method = $ride->resolveBookingMethodOption(
                                        $rideFeatureOptions['booking_method'] ?? [],
                                    );
                                @endphp
            ...
            <div class="col-span-1">
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-lg overflow-hidden shadow-3xl">
                        ...
                        <div class="grid divide-y">
                            @foreach ($ride->bookings->whereIn('status', [\App\Models\Booking::STATUS_BOOKED, \App\Models\Booking::STATUS_COMPLETED]) as $booking)
                                @if ($booking->passenger)
                                    <div
                                        class="flex flex-row items-center p-4 space-x-2 w-full no-scrollbar overflow-x-auto gap-2">
                                        <div>
                                            @include('partials.passenger_avatar', [
                                                'user' => $booking->passenger,
                                            ])
                                        ...
                                        <div
                                            ...
                                            <p class="font-semibold leading-4 text-md mb-0 whitespace-nowrap">
                                                {{ $booking->passenger->first_name }} </p>
                                            <div class="flex items-center space-x-2">
                                                <p class="text-gray-700 leading-4 md:mt-2 text-base whitespace-nowrap">
                                                ...
                                                <p class="text-gray-700 leading-4 md:mt-2 text-base whitespace-nowrap">
                                                    |</p>
                                                <p class="flex items-center gap-1 w-auto text-gray-700 leading-4 md:mt-2 text-base whitespace-nowrap">
                                                    @if ($booking->passenger->hasPassengerRatings())
                                                    <span>{{ number_format($booking->passenger->getPassengerAverageRating(), 1) }}</span>
                                        ...
                                        <div>
                                            @if ($ride->isCompleted() && !$booking->passenger->hasPassengerRatings())
                                            <a class="button-exp-fill" href="{{ route('review_passenger', ['lang' => app()->getLocale(), 'id' => $booking->id]) }}">
                    ...
                    @php
                        $cancellation_method = $ride->resolveBookingTypeOption(
                            $rideFeatureOptions['cancellation'] ?? [],
                        );
                    @endphp
                    <div class="bg-white rounded-lg overflow-hidden shadow-3xl">
                        <h3 class="bg-primary text-white py-2 px-4 text-2xl xl:text-3xl">
                ...
                <div
                    ...
                    <div class="px-4 pb-6 pt-4 sm:flex sm:flex-row-reverse sm:px-6 justify-center">
                        <button type="button" id="cancelRideResultClose"
                            class="inline-flex w-full justify-center rounded bg-red-500 px-3 py-2 font-FuturaMdCnBT text-lg font-medium text-white hover:text-white hover:shadow-lg shadow-sm hover:bg-red-400 sm:ml-3 sm:w-24">{{ $messages->popup_close_btn_text ?? 'Close' }}</button>
                    </div>
                </div>
            </div>
        </div>
