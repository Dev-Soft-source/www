...
class CoPassengerProvider extends GetConnect {
  ...
  Future getCoPassengers(rideId, token, langId) async {
    try {
      ...
      logger.info("Get Co-Passengers Response: ${response.body.toString()}");
      ...
      logger.error("HTTP error in getCoPassengers: $e");
      return Future.error({
        "type": "server",
        "message": "Server error occurred. Please try again later."
      });
    } catch (exception) {
      logger.error("Unknown error in getCoPassengers: $exception");
      return Future.error({"type": "unknown", "message": exception.toString()});
    }
  }
}
