...
class EditProfileController extends GetxController {
    ...
    await EditProfileProvider()
      ...
      if (resp['status'] != null && resp['status'] == "Success") {
        if (resp['data'] != null && resp['data']['user'] != null) {
          ...
          governmentImagePathOriginalOld.value =
              resp['data']['user']['government_issued_original_id'] ?? "";
          ...
          cityName.value =
              resp['data']['city'] != null ? resp['data']['city']['name'] : '';
        }
  ...
  void getImage(ImageSource imageSource) async {
    final croppedFile = await serviceController.imageCropper(
      imageSource,
    );
  ...
  editUserProfile(context, screenHeight) async {
    ...
    if (firstNameTextEditingController.text.isEmpty ||
      ...
      if (dobTextEditingController.text.isEmpty) {
        var message = validationMessageDetail['required'];
        message = message.replaceAll(
            ":Attribute", labelTextDetail['dob_error'] ?? 'Date of birth ');
        var err = {
          'title': "dob",
      ...
      if (countryName.value == "") {
        var message = validationMessageDetail['required'];
        message = message.replaceAll(
            ":Attribute", labelTextDetail['country_error'] ?? 'Country');
        var err = {
          'title': "country",
      ...
      if (stateName.value == "") {
        var message = validationMessageDetail['required'];
        message = message.replaceAll(
            ":Attribute", labelTextDetail['state_error'] ?? 'State');
        var err = {
          'title': "state",
    ...
    if (governmentImagePathOriginal.value != "") {
      ...
      if (sizeInMb > 10) {
        ...
        message = message.replaceAll(
            ":attribute",
            labelTextDetail['government_issued_error'] ??
                'government issue id');
    ...
    try {
      ...
      final stateForRequest =
          stateName.value == "Not Applicable" ? null : stateId.value;
      final cityForRequest =
          cityName.value == "Not Applicable" ? null : cityId.value;
      EditProfileProvider()
          .editUserProfile(
        ...
        if (resp['status'] != null && resp['status'] == "Error") {
          ...
          serviceController.loginUserDetail['gender'] =
              resp['data']['user']['gender'];
          serviceController.loginUserDetail['langId'] =
              resp['data']['user']['lang_id'];
          serviceController.langId.value =
              int.parse(resp['data']['user']['lang_id'].toString());
          ...
          logger
              .info("Login User Detail: ${serviceController.loginUserDetail}");

          Get.back();
          serviceController.showDialogue("Profile updated successfully",
              type: "success");
        }
        isOverlayLoading(false);
      }, onError: (err) {
        ...
        if (err is Map) {
          if (err.containsKey('type') && err['type'] == 'network') {
            message =
                "No internet connection. Please check your network and try again.";
