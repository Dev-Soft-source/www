                  ...
                  SingleChildScrollView(
                    child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          String otherUser = controller.userId.toString() ==
                                  controller.oldMessagesList[index]['sender']
                                          ['id']
                                      .toString()
                              ? "receiver"
                              : "sender";
                          return chatCard(
                              context: context,
                              image: controller.oldMessagesList[index]
                                      [otherUser]['profile_image'] ??
                                  "",
                              name:
                                  "${controller.oldMessagesList[index][otherUser]['first_name'] ?? ""} ${controller.oldMessagesList[index][otherUser]['last_name'] ?? ""}",
                              controller: controller,
                              time: controller.oldMessagesList[index]
                                      ['created_at']
                                  .substring(11, 16),
                              message: controller.oldMessagesList[index]
                                  ['message'],
                              numberOfMessages: 0,
                              onTap: () {
                                Get.toNamed(
                  ...
                  if (controller.oldMessagesList.isEmpty) ...[
                    Center(
                      ...
                      children: [
                        ...
                        txt20Size(
                            title:
                                "${controller.labelTextDetail['old_chat_page_no_messages_label'] ?? "You have no old messages"}",
                            context: context),
                      ],
                    )),
                  ]
