...
class TripDetailController extends GetxController {
    ...
    try {
      ...
      TripDetailProvider()
        ...
        if (resp['status'] != null && resp['status'] == "Error") {
          ...
          var getRideInfo = tripController.upComingRideList
              .firstWhereOrNull((element) => element['id'] == ride['id']);
          if (getRideInfo != null) {
          ...
          if (status == "accept") {
            ...
            double bookingCredit = bookingList
                .map((element) =>
                    double.tryParse(element['booking_credit'] ?? '') ?? 0.0)
                .reduce((a, b) => a + b);
            ...
            if (ride['total_amount'] > 0) {
              ride['booked_seats'] = (int.parse(ride['booked_seats'] != null
                          ? ride['booked_seats'].toString()
                          : '0') -
                      int.parse(resp['data']['booking']['seats'] != null
                          ? resp['data']['booking']['seats'].toString()
                          : "0"))
                  .toStringAsFixed(0);
              ride['fare'] = (double.parse(ride['fare'] != null
                          ? ride['fare'].toString()
                          : '0') -
                      double.parse(resp['data']['booking']['fare'] != null
                          ? resp['data']['booking']['fare'].toString()
                          : "0"))
                  .toStringAsFixed(1);
              ride['booking_fee'] = (double.parse(ride['booking_fee'] != null
                          ? ride['booking_fee'].toString()
                          : '0') -
                      double.parse(resp['data']['booking']['booking_credit'] !=
                              null
                          ? resp['data']['booking']['booking_credit'].toString()
                          : "0"))
                  .toStringAsFixed(1);
              ride['total_amount'] = (double.parse(ride['fare'] != null
                          ? ride['fare'].toString()
                          : '0') +
                      double.parse(ride['booking_fee'] ?? '0'))
                  .toStringAsFixed(1);
              ride.refresh();
              var tripController = Get.find<MyTripController>();
              var getRideInfo = tripController.upComingRideList
  ...
  enterCode(bookingId) {
    ...
    try {
      isOverlayLoading(true);
      TripDetailProvider()
          .enterCode(bookingId, securedCashTextEditingController.text,
        ...
        if (resp['status'] != null && resp['status'] == "Error") {
          ...
          var currentBooking = bookingList.firstWhereOrNull(
              (element) => element['id'].toString() == bookingId.toString());
          if (currentBooking != null) {
            currentBooking['secured_cash_attempt_count'] =
                resp['data'].toString();
          }
          ride.refresh();
        } else if (resp['status'] != null && resp['status'] == "Success") {
  ...
  noShowDriverData() {
    try {
      ...
      if (passengerList.isNotEmpty) {
        var bookingData = passengerList.firstWhereOrNull((element) =>
            element['user_id'] == serviceController.loginUserDetail['id']);
        if (bookingData != null) {
      ...
      TripDetailProvider()
          .noShowDriverData(ride['id'], 'driver', bookingId, ride['added_by'],
              serviceController.token)
          .then((resp) async {
        if (resp['status'] != null && resp['status'] == "Error") {
          serviceController.showDialogue(resp['message'].toString(),
    ...
    if (ride['payment_method_slug'] == "secured") {
      try {
        ...
        await TripDetailProvider()
            .checkPhoneNumber(
                serviceController.langId.value, serviceController.token)
            .then((resp) async {
  ...
  cancelRideByDriver() async {
    ...
    try {
      isOverlayLoading(true);
      await TripDetailProvider()
          .cancelMyBooking(ride['id'], serviceController.token)
          .then((resp) async {
        if (resp['status'] != null && resp['status'] == "Error") {
          serviceController.showDialogue(resp['message'].toString(),
              type: "error");
