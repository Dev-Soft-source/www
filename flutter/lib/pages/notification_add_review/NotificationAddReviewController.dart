        ...
        if (resp['status'] != null && resp['status'] == "Success") {
          if (resp['data'] != null && resp['data']['ride'] != null) {
            ...
            if (passengerBookingId.value != '0') {
              ...
              var getBooking = bookingList.firstWhereOrNull((element) =>
                  element['id'] ==
                  int.parse(passengerBookingId.value.toString()));
              if (getBooking != null) {
                reviewPassengerImage.value =
                    getBooking['passenger']['profile_image'];
                reviewPassengerName.value =
                    "${getBooking['passenger']['first_name']}";
              }
      ...
      await MyTripProvider()
        ...
        if (resp['status'] != null && resp['status'] == "Error") {
          ...
          if (resp['data']['rating'] != null) {
            bool isMyTripControllerRegistered =
                Get.isRegistered<MyTripController>();
          ...
          if (notificationId != '0') {
            bool isNotificationControllerRegistered =
                Get.isRegistered<NotificationController>();
            if (isNotificationControllerRegistered == true) {
              var tempController = Get.find<NotificationController>();
              tempController.notificationsList.removeWhere((element) =>
                  element['id'] == int.parse(notificationId.toString()));
    ...
    try {
      ...
      await MyTripProvider()
        ...
        if (resp['status'] != null && resp['status'] == "Error") {
          ...
          if (isMyTripControllerRegistered == true) {
            var tempController = Get.find<MyTripController>();
            var rideData = tempController.completedRideList
