    ...
    return Scaffold(
        appBar: AppBar(
          ...
          title: Obx(() => secondAppBarWidget(
              context: context,
              title:
                  "${controller.labelTextDetail['top_up_main_heading'] ?? "Top up my balance"}")),
                      ...
                      children: [
                        Row(
                          children: [
                            txt20Size(
                                title:
                                    "${controller.labelTextDetail['purchase_top_up_label'] ?? "Purchase top up balance"}",
                                fontFamily: regular,
                                context: context),
                            // Asterisk should be slightly smaller (18px)
                            txt18Size(
                                title: "*",
                                fontFamily: regular,
                                context: context,
                        ...
                        fieldsWidget(
                          maxLength: 30,
                          textController: controller.drAmountController,
                          onChanged: (value) async {
                            controller.gPayAmount.value =
                          ...
                          fontSize: 18.0,
                          placeHolder:
                              "${controller.labelTextDetail['purchase_top_up_placeholder'] ?? "Enter the amount you want to add"}",
                          isError: controller.errors.firstWhereOrNull(
                                  (element) => element['title'] == "amount") !=
                              null,
                        ),
                        ...
                        30.heightBox,
                        // "Pay with" is a normal label → 20px
                        txt20Size(
                            title:
                                "${controller.labelTextDetail['pay_with_label'] ?? 'Pay with'}",
                            fontFamily: regular,
                            context: context,
                        ...
                        paymentOptionCard(
                            ...
                            img: visa,
                            title:
                                "${controller.labelTextDetail['credit_card_label'] ?? 'Credit Card'}",
                            iconHeight: 18.0,
                            iconWidth: 86.0,
                            onTap: () async {
                              ...
                              ? SizedBox(
                                  ...
                                  child: ApplePayButton(
                                    ...
                                    onPaymentResult: (result) async {
                                      ...
                                      final decodedToken =
                                          jsonDecode(rawTokenJson);
                                      final token = decodedToken['id'];
                                      await controller
                                          .getGooglePayApplePay(token);
                                    },
