...
class PostRideAgainController extends GetxController
  ...
  getMorePostRideList() async {
    try {
      type.value == "upcoming"
          ? isScrollUpcomingLoading(true)
          : type.value == "completed"
              ? isScrollCompletedLoading(true)
              : isScrollCancelledLoading(true);
      await PostRideAgainProvider()
          .getPostRideList(
              serviceController.token,
              type.value,
              ...
              serviceController.langId.value)
          .then((resp) async {
        if (resp['data']['rides'] != null &&
            resp['data']['rides']['data'].isNotEmpty) {
        } else {
          type.value == "upcoming"
              ? isScrollUpcomingLoading(false)
              : type.value == "completed"
                  ? isScrollCompletedLoading(false)
                  : isScrollCancelledLoading(false);
          type.value == "upcoming"
              ? upcomingEnd.value = "No more data found"
              : type.value == "completed"
                  ? completedEnd.value = "No more data found"
                  : cancelledEnd.value = "No more data found";
        }
        if (resp['data']['rides'] != null &&
            resp['data']['rides']['data'] != null) {
