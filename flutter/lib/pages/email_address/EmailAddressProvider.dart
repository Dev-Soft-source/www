...
class EmailAddressProvider extends GetConnect {
  ...
  Future updateEmailAddress(
      email, newEmail, confirmEmail, token, userId) async {
    try {
      final data = {
        "_method": "PUT",
      ...
      final response = await getConnect
          .post("$baseUrl/$updateUserEmail?id=$userId", data, headers: {
      ...
      logger.error("HTTP error in updateEmailAddress: $e");
      return Future.error({
        "type": "server",
        "message": "Server error occurred. Please try again later."
      });
    } catch (exception) {
      logger.error("Unknown error in updateEmailAddress: $exception");
