...
class ProfileSettingController extends GetxController {
  ...
  Future<void> loadInitialData() async {
    try {
      ...
      if (error is Map &&
        ...
        errorStateManager.setError(
          "Unable to load profile settings. Please check your connection and try again.",
          ErrorType.unknown,
          loadInitialData,
        );
      }
  ...
  Future<void> getLabelTextDetail() async {
    await StageProvider()
        .getLabelTextDetail(serviceController.langId.value, profileSetting,
            serviceController.token)
        .then((resp) async {
      if (resp['status'] != null && resp['status'] == "Success") {
        if (resp['data'] != null &&
            resp['data']['profileSettingPage'] != null) {
