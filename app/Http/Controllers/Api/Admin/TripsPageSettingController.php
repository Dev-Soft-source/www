...
class TripsPageSettingController extends Controller
{
    ...
    public function uploadExcel(Request $request)
    {
        try {
            ...
            try {
                Excel::import(new TripsPageSettingImport($request->language_id), $request->file('excel_file'));
                return $this->successResponse(['language' => $language->name], "Trips page settings for {$language->name} uploaded successfully from Excel.");
    ...
    {
        try {
            return Excel::download(new TripsPageSettingTemplateExport($request->get('format', 'single_column')),
