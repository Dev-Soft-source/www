...
class MessageController extends Controller
{
    ...
    public function index()
    {
        try{
            ...
            return $this->apiSuccessResponse(ContactMessageResource::collection($messages), 'Data Get Successfully!');
        } catch (\Exception $e) {
    ...
    protected function whereClause($messages)
    {
        if (isset($_GET['searchParam']) && $_GET['searchParam'] != '') {
            $messages = $messages->where('name', 'LIKE', '%' . $_GET['searchParam'] . '%')->orWhere('email', 'LIKE', '%' . $_GET['searchParam'] . '%')->orWhere('id', 'LIKE', '%' . $_GET['searchParam'] . '%');
        }
        return $messages;
    }
