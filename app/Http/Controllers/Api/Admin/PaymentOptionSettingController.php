...
class PaymentOptionSettingController extends Controller
{
    ...
    public function uploadExcel(Request $request)
    {
        try {
            ...
            try {
                Excel::import(new PaymentOptionSettingImport($request->language_id), $request->file('excel_file'));
                return $this->successResponse(['language' => $language->name], "Payment option settings for {$language->name} uploaded successfully from Excel.");
    ...
    {
        try {
            return Excel::download(new PaymentOptionSettingTemplateExport($request->get('format', 'single_column')),
