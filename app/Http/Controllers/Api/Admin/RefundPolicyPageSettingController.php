...
class RefundPolicyPageSettingController extends Controller
{
    ...
    public function uploadExcel(Request $request)
    {
        try {
            ...
            try {
                Excel::import(new RefundPolicyPageSettingImport($request->language_id), $request->file('excel_file'));
    ...
    {
        try {
            return Excel::download(new RefundPolicyPageSettingTemplateExport($request->get('format', 'single_column')),
                'refund_policy_page_settings_template_' . date('Y-m-d') . '.xlsx');
        } catch (\Exception $e) {
