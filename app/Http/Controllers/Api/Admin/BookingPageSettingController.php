...
class BookingPageSettingController extends Controller
{
    ...
    /**
     * Upload booking page settings via Excel file (all-languages format: Field Name + one column per language).
     */
    public function uploadExcel(Request $request)
    {
        try {
            ...
            try {
                $import = new BookingPageSettingImport(null);
                Excel::import($import, $request->file('excel_file'));

                return $this->successResponse(
                    [],
                    'Booking page settings for all languages uploaded successfully from Excel.'
                );
    ...
    public function downloadTemplate(Request $request)
    {
        try {
            ...
            if ($format === 'all_languages') {
                ...
                $existingData = BookingPageSetting::with('bookingPageSettingDetail')->first();
            }
            $fileName = 'booking_page_settings_template_' . date('Y-m-d') . '.xlsx';
            return Excel::download(
                new BookingPageSettingTemplateExport($format, $languages, $existingData),
                $fileName
            );
        } catch (\Exception $e) {
