...
class ProfilePageSettingController extends Controller
{
    ...
    /**
     * Upload profile page settings via Excel file.
    ...
    public function uploadExcel(Request $request)
    {
        try {
            ...
            try {
                Excel::import(new ProfilePageSettingImport($isAllLanguages ? null : $request->language_id), $request->file('excel_file'));

                if ($isAllLanguages) {
                    return $this->successResponse([], 'Profile page settings for all languages uploaded successfully from Excel.');
                }
    ...
    public function downloadTemplate(Request $request)
    {
        try {
            ...
            if ($format === 'all_languages') {
                ...
                $existingData = ProfilePageSetting::with('profilePageSettingDetail')->first();
            }

            $fileName = 'profile_page_settings_template_' . date('Y-m-d') . '.xlsx';

            return Excel::download(
                new ProfilePageSettingTemplateExport($format, $languages, $existingData),
                $fileName
            );
        } catch (\Exception $e) {
