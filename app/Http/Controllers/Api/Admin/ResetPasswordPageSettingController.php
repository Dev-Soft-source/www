...
class ResetPasswordPageSettingController extends Controller
{
    ...
    public function uploadExcel(Request $request)
    {
        try {
            ...
            try {
                Excel::import(new ResetPasswordPageSettingImport($request->language_id), $request->file('excel_file'));
    ...
    {
        try {
            return Excel::download(new ResetPasswordPageSettingTemplateExport($request->get('format', 'single_column')),
                'reset_password_page_settings_template_' . date('Y-m-d') . '.xlsx');
        } catch (\Exception $e) {
