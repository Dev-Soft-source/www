...
class RideController extends Controller
{
    ...
    /**
     * Stateless web {@see WebRideController} instance for post-ride normalization, validation, and {@see RidePostService} persistence hooks.
     */
    protected function webRidePersistence(): WebRideController
    {
    ...
    public function _PostRideStore(Request $request, $ride_id = 0)
    {
        ...
        $existingRide = $ride_id ? Ride::where('id', $ride_id)->where('added_by', $user->id)->with('detail')->first() : null;
        ...
        $webRide->appendStopDepartureAtValidation($request, $validator);
        if ($validator->fails()) {
        ...
        if (!$persist->ok) {
            return $this->errorResponse($persist->errorMessage);
        }

        if (!$ride_id) {
            $langAbbr = optional($this->selectedLanguage)->abbreviation
                ?? Language::where('is_default', 1)->value('abbreviation')
                ?? 'en';
            app(RideWebNotificationController::class)->dispatchPostRidePostedNotifications(
                $persist->ride->id,
                $user->id,
                [
                    'my_rides_lang_abbr' => $langAbbr,
        ...
        $initialRide = Ride::with(['detail', 'rideStops', 'rideStopSegments'])->find($persist->ride->id);
    ...
    public function _UpdateRide(Request $request, $ride_id = null)
    {
        $resolvedId = (int) (
        ...
        return $this->_PostRideStore($request, $resolvedId);
    }
    ...
    protected function normalizeAppPostRideRequest(Request $request, ?Ride $ride = null): void
    {
        ...
        if (!$request->filled('vehicle_mode')) {
            // Prefer explicit skip first: app always sends vehicle_id (may be stale) even when skipping.
            if ($this->isTruthyAppValue($request->input('skip_vehicle'))) {
