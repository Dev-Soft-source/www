...
class CostSharingPageSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    ...
    {
        $setting = CostSharingPageSetting::first() ?? CostSharingPageSetting::create([]);
        if ($rows->isEmpty()) {
            Log::warning('No rows found in Cost Sharing Page Excel file');
            return;
        }
        ...
        if ($isAllLanguages) {
            ...
            Log::info('Cost Sharing Page Settings Excel import (all languages) completed successfully');
            return;
        }
    ...
    protected function processAllLanguagesFormat(CostSharingPageSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $value = $row[$col] ?? null;
                $detail = CostSharingPageSettingDetail::firstOrCreate(
                    [
                        'cost_sharing_page_id' => $setting->id,
    ...
    protected function applyData(CostSharingPageSetting $setting, array $data): void
    {
        $payload = [
            'cost_sharing_page_id' => $setting->id,
        ...
        CostSharingPageSettingDetail::updateOrCreate(
            ['cost_sharing_page_id' => $setting->id, 'language_id' => $this->languageId],
            $payload
        );
    }
