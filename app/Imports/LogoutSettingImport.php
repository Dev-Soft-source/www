...
class LogoutSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    ...
    public function collection(Collection $rows)
    {
        ...
        if (!$setting) {
            $setting = LogoutSetting::create([]);
        }
        if ($rows->isEmpty()) {
            Log::warning('No rows found in Logout Excel file');
            return;
        }
        ...
        if ($isAllLanguages) {
            ...
            Log::info('Logout Settings Excel import (all languages) completed successfully');
            return;
        }
    ...
    protected function processAllLanguagesFormat(LogoutSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $value = $row[$col] ?? null;
                $detail = LogoutSettingDetail::firstOrCreate(
                    [
