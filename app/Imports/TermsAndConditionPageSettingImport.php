...
class TermsAndConditionPageSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    ...
    {
        $setting = TermsAndConditionPageSetting::first() ?? TermsAndConditionPageSetting::create([]);
        ...
        $payload = [
            'terms_page_id' => $setting->id,
            'language_id' => $this->languageId,
        ];
        foreach ($this->fields() as $f) { $payload[$f] = $data[$f] ?? null; }

        TermsAndConditionPageSettingDetail::updateOrCreate(
            ['terms_page_id' => $setting->id, 'language_id' => $this->languageId],
            $payload
        );
    }
     ...
     */
    protected function processAllLanguagesFormat(TermsAndConditionPageSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $detail = TermsAndConditionPageSettingDetail::firstOrNew([
                    'terms_page_id' => $setting->id,
