...
class MyDriverSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    ...
    public function collection(Collection $rows)
    {
        ...
        if (!$setting) {
            $setting = DriverSetting::create([]);
        }
        if ($rows->isEmpty()) {
            Log::warning('No rows found in My Driver License Excel file');
            return;
        }
        ...
        if ($isAllLanguages) {
            ...
            Log::info('My Driver License Settings Excel import (all languages) completed successfully');
            return;
        }
    ...
    protected function processAllLanguagesFormat(DriverSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $value = $row[$col] ?? null;
                $detail = DriverSettingDetail::firstOrCreate(
                    [
