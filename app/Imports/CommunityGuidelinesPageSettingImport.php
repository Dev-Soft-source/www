...
class CommunityGuidelinesPageSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    /** @var int|null */
    ...
    {
        $setting = CommunityGuidelinesPageSetting::first() ?? CommunityGuidelinesPageSetting::create([]);
        if ($rows->isEmpty()) {
            return;
        }
        ...
        $payload = [
            'community_guidelines_page_id' => $setting->id,
        ...
        CommunityGuidelinesPageSettingDetail::updateOrCreate(
            ['community_guidelines_page_id' => $setting->id, 'language_id' => $this->languageId],
            $payload
        );
    }

    protected function processAllLanguagesFormat(CommunityGuidelinesPageSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $detail = CommunityGuidelinesPageSettingDetail::firstOrNew([
                    'community_guidelines_page_id' => $setting->id,
