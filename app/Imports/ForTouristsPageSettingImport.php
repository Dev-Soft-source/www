...
class ForTouristsPageSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    /** @var int|null */
    ...
    {
        $setting = ForTouristsPageSetting::first() ?? ForTouristsPageSetting::create([]);
        if ($rows->isEmpty()) {
            return;
        }
        ...
        $payload = [
            'for_tourists_page_id' => $setting->id,
        ...
        ForTouristsPageSettingDetail::updateOrCreate(
            ['for_tourists_page_id' => $setting->id, 'language_id' => $this->languageId],
            $payload
        );
    }

    protected function processAllLanguagesFormat(ForTouristsPageSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $detail = ForTouristsPageSettingDetail::firstOrNew([
                    'for_tourists_page_id' => $setting->id,
