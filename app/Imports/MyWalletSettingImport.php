...
class MyWalletSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    ...
    public function collection(Collection $rows)
    {
        ...
        if ($rows->isEmpty()) {
            Log::warning('No rows found in My Wallet Excel file');
            return;
        }
        ...
        if ($isAllLanguages) {
            ...
            Log::info('My Wallet Settings Excel import (all languages) completed successfully');
            return;
        }
    ...
    protected function processAllLanguagesFormat(MyWalletSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $value = $row[$col] ?? null;
                $detail = MyWalletSettingDetail::firstOrCreate(
                    [
