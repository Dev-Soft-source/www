...
class HomePageSettingImport implements ToCollection, WithHeadingRow, WithValidation
{
    ...
    public function collection(Collection $rows)
    {
        ...
        if ($rows->isEmpty()) {
            Log::warning('No rows found in Home Page Excel file');
            return;
        }
        ...
        if ($isAllLanguages) {
            ...
            Log::info('Home Page Settings Excel import (all languages) completed successfully');
            return;
        }
    ...
    protected function processAllLanguagesFormat(HomePageSetting $setting, Collection $rows): void
    {
        $firstRow = $rows->first();
        ...
        foreach ($rows as $row) {
            ...
            foreach ($languageColumns as $col) {
                ...
                $value = $row[$col] ?? null;
                $detail = HomePageSettingDetail::firstOrCreate(
                    [
