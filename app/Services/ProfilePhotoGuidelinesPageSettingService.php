...
class ProfilePhotoGuidelinesPageSettingService
{
    public function validation($languages, $validationRule, $errorMessages)
    {
        ...
        foreach ($languages as $language) {
            if ($language->is_default == '1') {
                ...
                $validationRule = array_merge($validationRule, ['example_label.example_label_' . $language->id => ['nullable', 'string']]);
            }
        }
    ...
    public function fields($profilePhotoGuidelinesPageSetting, $language, $request)
    {
        return [
            'profile_photo_guidelines_page_id' => $profilePhotoGuidelinesPageSetting->id,
            ...
            'example_label' => $this->data($request, $language, 'example_label'),
        ];
    }

    public function update($profilePhotoGuidelinesPageSetting, $language, $request)
    {
        $fields = $this->fields($profilePhotoGuidelinesPageSetting, $language, $request);
        $exists = ProfilePhotoGuidelinesPageSettingDetail::whereProfilePhotoGuidelinesPageId($profilePhotoGuidelinesPageSetting->id)->whereLanguageId($language->id)->exists();
