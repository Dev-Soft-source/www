...
class PaymentOptionSettingTemplateExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths
{
    protected $format;
    ...
    /** @var \App\Models\PaymentSetting|null */
    ...
    {
        $fields = ['name','mobile_default_card_tab','mobile_card_name_label','main_heading','mobile_card_number_label','mobile_expiry_date_label','delete_card_button_text','add_new_card_button_text','no_payment_message','set_primary_card_label','select_card_type_text'];
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = static::getTranslatableFieldsWithDefaults();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('paymentSettingDetail')) {
            foreach ($this->existingData->paymentSettingDetail as $d) {
