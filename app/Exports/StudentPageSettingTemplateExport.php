...
class StudentPageSettingTemplateExport implements FromCollection, WithHeadings, ShouldAutoSize
{
    protected $format;
    ...
    /** @var \App\Models\StudentPageSetting|null */
     ...
     * @param \App\Models\StudentPageSetting|null $existingData For all_languages (with studentPageSettingDetail loaded)
     */
    ...
    public function collection(): Collection
    {
        ...
        $values = [];
        if ($setting = StudentPageSetting::with('studentPageSettingDetail')->first()) {
            $detail = optional($setting->studentPageSettingDetail)->first();
            if ($detail) {
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = $this->getFields();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('studentPageSettingDetail')) {
            foreach ($this->existingData->studentPageSettingDetail as $d) {
