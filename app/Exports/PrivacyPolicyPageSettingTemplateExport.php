...
class PrivacyPolicyPageSettingTemplateExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths
{
    protected $format;
    ...
    /** @var \App\Models\PrivacyPolicyPageSetting|null */
    ...
    {
        return array_fill_keys(['name', 'meta_keywords', 'meta_description', 'main_heading', 'main_text'], '');
    }
    ...
    protected function singleColumnFormat(): Collection
    {
        ...
        foreach ($fields as $field => $default) {
            $rows[] = ['field_name' => $field, 'translation_value' => $default];
        }
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = static::getTranslatableFieldsWithDefaults();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('privacyPolicyPageSettingDetail')) {
            foreach ($this->existingData->privacyPolicyPageSettingDetail as $d) {
    ...
    {
        if ($this->format === 'single_column') return ['A' => 25, 'B' => 80];
        if ($this->format === 'all_languages') {
