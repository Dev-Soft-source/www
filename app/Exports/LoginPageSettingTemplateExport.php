...
class LoginPageSettingTemplateExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths
{
    protected $format;
    ...
    /** @var \App\Models\LoginPageSetting|null */
     ...
     * @param \App\Models\LoginPageSetting|null $existingData - with loginPageSettingDetail loaded
     */
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = static::getTranslatableFieldsWithDefaults();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('loginPageSettingDetail')) {
            foreach ($this->existingData->loginPageSettingDetail as $d) {
