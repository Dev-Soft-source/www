...
class TermsAndConditionPageSettingTemplateExport implements FromCollection, WithHeadings, ShouldAutoSize
{
    protected $format;
    ...
    /** @var \App\Models\TermsAndConditionPageSetting|null */
     ...
     * @param \App\Models\TermsAndConditionPageSetting|null $existingData For all_languages (with termsAndConditionPageSettingDetail loaded)
     */
    ...
    public function collection(): Collection
    {
        ...
        $values = [];
        if ($setting = TermsAndConditionPageSetting::with('termsAndConditionPageSettingDetail')->first()) {
            $detail = optional($setting->termsAndConditionPageSettingDetail)->first();
            if ($detail) {
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = $this->getFields();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('termsAndConditionPageSettingDetail')) {
            foreach ($this->existingData->termsAndConditionPageSettingDetail as $d) {
