...
class DisclaimerPageSettingTemplateExport implements FromCollection, WithHeadings, ShouldAutoSize
{
    protected $format;
    ...
    public function collection(): Collection
    {
        ...
        $values = [];
        if ($setting = DisclaimerPageSetting::with('disclaimerPageSettingDetail')->first()) {
            $detail = optional($setting->disclaimerPageSettingDetail)->first();
            if ($detail) {
                foreach ($fields as $f) {
                    $values[$f] = $detail->{$f} ?? '';
                }
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = $this->getFields();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('disclaimerPageSettingDetail')) {
            foreach ($this->existingData->disclaimerPageSettingDetail as $d) {
