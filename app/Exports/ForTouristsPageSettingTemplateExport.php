...
class ForTouristsPageSettingTemplateExport implements FromCollection, WithHeadings, ShouldAutoSize
{
    protected $format;
    ...
    public function collection(): Collection
    {
        ...
        $values = [];
        if ($setting = ForTouristsPageSetting::with('forTouristsPageSettingDetail')->first()) {
            $detail = optional($setting->forTouristsPageSettingDetail)->first();
            if ($detail) {
                foreach ($fields as $f) {
                    $values[$f] = $detail->{$f} ?? '';
                }
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = $this->getFields();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('forTouristsPageSettingDetail')) {
            foreach ($this->existingData->forTouristsPageSettingDetail as $d) {
