...
class MyReviewSettingTemplateExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths
{
    protected $format;
    ...
    /** @var \App\Models\MyReviewSetting|null */
     ...
     * @param \App\Models\MyReviewSetting|null $existingData - with reviewSettingDetail loaded
     */
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = static::getTranslatableFieldsWithDefaults();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('reviewSettingDetail')) {
            foreach ($this->existingData->reviewSettingDetail as $d) {
