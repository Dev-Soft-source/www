...
class DisputePageSettingTemplateExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths
{
    protected $format;
    ...
    /** @var \App\Models\DisputePageSetting|null */
     ...
     * @param \App\Models\DisputePageSetting|null $existingData - with disputePageSettingDetail loaded
     */
    ...
    public static function getTranslatableFieldsWithDefaults(): array
    {
        return [
            'name' => 'Dispute Policy',
            'meta_keywords' => 'dispute, policy, resolution',
            'meta_description' => 'Learn about our dispute policy',
            'main_heading' => 'Dispute Policy',
            'main_text' => 'Please read our dispute policy carefully.',
        ];
    }
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = static::getTranslatableFieldsWithDefaults();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('disputePageSettingDetail')) {
            foreach ($this->existingData->disputePageSettingDetail as $d) {
