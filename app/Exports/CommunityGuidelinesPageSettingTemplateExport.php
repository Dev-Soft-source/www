...
class CommunityGuidelinesPageSettingTemplateExport implements FromCollection, WithHeadings, ShouldAutoSize
{
    protected $format;
    ...
    public function collection(): Collection
    {
        ...
        $values = [];
        if ($setting = CommunityGuidelinesPageSetting::with('communityGuidelinesPageSettingDetail')->first()) {
            $detail = optional($setting->communityGuidelinesPageSettingDetail)->first();
            if ($detail) {
                foreach ($fields as $f) {
                    $values[$f] = $detail->{$f} ?? '';
                }
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $fields = $this->getFields();
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('communityGuidelinesPageSettingDetail')) {
            foreach ($this->existingData->communityGuidelinesPageSettingDetail as $d) {
