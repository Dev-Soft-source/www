...
class RefundPolicyPageSettingTemplateExport implements FromCollection, WithHeadings, ShouldAutoSize, WithStyles, WithColumnWidths
{
    protected $format;
    ...
    /** @var \App\Models\RefundPolicyPageSetting|null */
     ...
     * @param \App\Models\RefundPolicyPageSetting|null $existingData - For all_languages (with refundPolicyPageSettingDetail loaded)
     */
    ...
    public static function getTranslatableFieldsWithDefaults(): array
    {
        return [
            ...
            'meta_description' => 'Refund policy page',
            'main_heading' => 'Refund Policy',
            'main_text' => 'Refund policy content...',
        ];
    }
    ...
    protected function allLanguagesFormat(): Collection
    {
        ...
        $detailsByLang = [];
        if ($this->existingData && $this->existingData->relationLoaded('refundPolicyPageSettingDetail')) {
            foreach ($this->existingData->refundPolicyPageSettingDetail as $d) {
